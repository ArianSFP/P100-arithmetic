...
template<bool Wide> __device__ __forceinline__ float activation(const uint16_t *a,int i) {
    if constexpr(Wide)return reinterpret_cast<const float *>(a)[i];
    else return widen(a[i]);
}
...
template<bool Repacked,bool Wide=false> __device__ __forceinline__ void baseline(ARGS) {
    ...
    for(int g=lane;g<groups;g+=32) {
        ...
        for(int j=0;j<32;++j) {
            ...
            dot=__fmaf_rn(float(q),activation<Wide>(a,(batch*groups+g)*32+j),dot);
        }
...
extern "C" __global__ void native_direct_wide(ARGS) {baseline<false,true>(PASS);}
extern "C" __global__ void old_repacked_wide(ARGS) {baseline<true,true>(PASS);}

template<int R,int Method,bool Wide=false> __device__ __forceinline__ void row_body(ARGS) {
    ...
    for(int g=blockIdx.z;g<groups;g+=gridDim.z) {
        ...
        if constexpr(Method<2) {
            #pragma unroll
            for(int j=0;j<32;++j) {
                float av=activation<Wide>(a,(batch*groups+g)*32+j);
                #pragma unroll
                for(int r=0;r<R;++r) {
                    int q;
            ...
            for(int q=0;q<8;++q) {
                float entry=0;
                #pragma unroll
                for(int j=0;j<4;++j)if(lane&(1<<j))entry=__fadd_rn(entry,activation<Wide>(a,(batch*groups+g)*32+4*q+j));
                #pragma unroll
                for(int r=0;r<R;++r) {
                    #pragma unroll
                    for(int p=0;p<4;++p) {
...
#define WIDE_ROW(NAME,R,M) extern "C" __global__ void NAME##_wide(ARGS) {row_body<R,M,true>(PASS);}
WIDE_ROW(direct_r1,1,0) WIDE_ROW(direct_r2,2,0) WIDE_ROW(direct_r4,4,0)
WIDE_ROW(plane_direct_r1,1,1) WIDE_ROW(plane_direct_r2,2,1) WIDE_ROW(plane_direct_r4,4,1)
WIDE_ROW(lut_r1,1,2) WIDE_ROW(lut_r2,2,2) WIDE_ROW(lut_r4,4,2)
...
extern "C" __global__ void widen_inputs(const uint16_t *a,float *out,int count) {
    int i=blockIdx.x*blockDim.x+threadIdx.x;
    if(i<count)out[i]=widen(a[i]);
}
