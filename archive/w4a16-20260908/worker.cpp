...
struct Config {
    ...
    int base()const{return method%5;}
    bool wide()const{return method>=5;}
    int format()const{return base()>=3;}
...
std::vector<Config> configs(bool sweep) {
    ...
    for(size_t i=0;i<count;++i)out.push_back({out[i].method+5,out[i].reuse,out[i].split});
    return out;
}
...
struct Dataset {
    ...
    void launch(const Config &c) {
        ...
        void *args[]={&dn.p,&w,&s,&act,&dst,&m,&groups};
...
void benchmark(int m,int k,int n,bool sweep,const std::string &path) {
    ...
    for(const auto &c:cfg)require(c.method>=0 && c.method<=9 && (c.reuse==1 || c.reuse==2 || c.reuse==4) && c.split>=1 && c.split<=64 && !(c.split&(c.split-1)) && (c.base()>=2 || (c.split==32 && c.reuse==1)),"configuration bounds");
