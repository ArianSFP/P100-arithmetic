...
CUmodule module,original_module;
...
CUfunction function(const std::string &name) {
    ...
    CUfunction f;CK(cuModuleGetFunction(&f,name.rfind("r2_",0)==0 ? module : original_module,name.c_str()));functions[name]=f;return f;
}
struct Buffer {
...
struct Config {
    ...
    int warps()const{return method==18 || method==21 || method==33 ? 1 : method==19 || method==22 || method==32 ? 2 : method==20 || method==23 ? 8 : 4;}
    int batch_reuse()const{return method==31 ? 2 : method>=30 ? 4 : 1;}
...
std::vector<Config> configs(bool sweep) {
    (void)sweep;std::vector<Config> out={{2,2,32},{2,4,32}};
    for(int m=10;m<=23;++m)for(int r:{2,4,6,8})out.push_back({m,r,32});
    for(int m=30;m<=36;++m)for(int r:{1,2,4})out.push_back({m,r,32});
    return out;
}
...
struct Dataset {
    ...
    void launch(const Config &c) {
        CUdeviceptr w=0,s=0,dst=c.base()<2 || c.split==1 ? output.p : partial.p;
        ...
        if(c.base()<2)CK(cuLaunchKernel(function(c.name()),(m+7)/8,n,1,256,1,1,0,nullptr,args,nullptr));
            ...
            CK(cuLaunchKernel(function(c.name()),(m+threads*c.reuse-1)/(threads*c.reuse),(n+c.batch_reuse()-1)/c.batch_reuse(),c.split,threads,1,1,0,nullptr,args,nullptr));
...
int main(int argc,char **argv) {
    ...
    try {
        ...
        CK(cuModuleLoad(&original_module,"/home/arian/hfma2-gp100-probe/w4a16-20260908/build/kernels.cubin"));
        if(mode=="validate" || mode=="smoke")correctness(mode=="smoke");else benchmark(m,k,n,mode=="sweep",path);
        CK(cuCtxSynchronize());CK(cuModuleUnload(module));CK(cuModuleUnload(original_module));CK(cuCtxDestroy(ctx));
