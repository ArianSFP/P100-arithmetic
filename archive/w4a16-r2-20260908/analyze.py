    ...
    for path in sorted((ROOT/'gpu-results').glob('*/result.json')):
        ...
        for line in log.splitlines():
            ...
            if line.startswith('TIME '):
                ...
                checks[key(d)]=d
                assert int(d['S'])==32 and int(d['baseline_changed'])==0,path
        ...
        out.append(dict(run=path.parent.name,gpu=rec['gpu'],shape=shapes.pop(),record=rec,checks=checks,packs=packs,
...
def freeze():
    ...
    for r in runs:
        assert r['gpu']==1,'Only GPU1 selects candidates'
    ...
    # Sealed controls and fixed component comparisons, selected before repeats.
    chosen={(2,2,32),(2,4,32),(10,4,32),(11,4,32),(12,4,32),
            (13,8,32),(14,4,32),(17,4,32),(30,4,32),(31,4,32)}
    selections=[]
    for s in SHAPES:
        run=byshape[s][0];med=run['medians']
        eligible=[k for k in med if k[0]>=10]
        win=min(eligible,key=med.get);chosen.add(win);winners={'strict':dict(config=win,us=med[win])}
...
def aggregate():
    ...
    for r in runs:
        ...
        byshape[r['gpu'],r['shape']].append(r)
    assert set(byshape)=={(g,s) for g in (1,2,3) for s in SHAPES} and all(len(v)==3 for v in byshape.values()),'Need exactly three fresh workers per GPU/shape'
    rows=[];allchecks=0;retained=0
    for gpu,s in sorted(byshape):
        for k in sorted(configs):
            reps=byshape[gpu,s];medians=[r['medians'][k] for r in reps];checks=[r['checks'][k] for r in reps]
            assert all(c==checks[0] for c in checks),'Seed/correctness changed across repetitions'
            allchecks+=3;retained+=24
            rows.append(dict(gpu=gpu,M=s[0],K=s[1],N=s[2],method=k[0],R=k[1],S=k[2],
    ...
    result=dict(manifest=manifest,worker_count=len(runs),configuration_checks=allchecks,retained_event_samples=retained,
                ...
                packing=[dict(run=r['run'],gpu=r['gpu'],shape=r['shape'],records=r['packs']) for r in runs])
    ...
    for gpu,s in sorted(byshape):
        cells=[r for r in rows if (r['gpu'],(r['M'],r['K'],r['N']))==(gpu,s)]
        print('GPU',gpu,'SHAPE',s)
