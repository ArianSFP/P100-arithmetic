...
for projection in ('gu','down'):
    for repetition in range(3):
        ...
        assert raw.count('bad=0 nonfinite=0')==3,tag
        assert 'wire=f32' in raw and 'mode=w16-dualrail' in raw and 'diff_vs_w8=0' in raw,tag
        times={mode:[] for mode in ('original-q8','w8-dualrail','w16-dualrail')}
        for mode,us in re.findall(r'TIME mode=(\S+) rep=\d+ us=([0-9.]+)',raw):
            times[mode].append(float(us))
...
out={'workers':workers,'summary':summary}
