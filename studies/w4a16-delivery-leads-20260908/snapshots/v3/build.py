...
def main():
    ...
    if(c.kind==-3){''')
    s=replace_once(s,'    std::string only;', '    int approved=0;\n    std::string only;')
    s=replace_once(s,'        if(key=="--tokens")t=v;', '        if(key=="--gpu-approved")approved=v;else if(key=="--tokens")t=v;')
    ...
    for(int u:{4,8,16})cfg.push_back({32,64,128,1,1,u,1,"delivery_m32u"+std::to_string(u),2010+u});''')
