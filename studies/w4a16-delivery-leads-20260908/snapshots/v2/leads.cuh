...
void delivery_lead(const void *ap,const unsigned char *w,float *out,int t,int n,int k,int experts) {
    ...
    constexpr int RM=SWAP?4:8,CN=SWAP?8:4;
    const int ar=SWAP?warp*16+(lane/8)*4:(warp/2)*32+(lane/8)*8;
    const int bc=SWAP?(lane%8)*8:(warp%2)*32+(lane%8)*4;
        ...
        for(int g=0;g<groups;++g) {
            ...
            for(int chunk=0;chunk<32/U;++chunk) {
                ...
                for(int s=0;s<U;++s) {
                    ...
                    float a[RM],b[CN];
                    #pragma unroll
                    for(int i=0;i<RM;i+=(SWAP?4:2)) {
                        if constexpr(SWAP)*reinterpret_cast<float4*>(a+i)=*reinterpret_cast<const float4*>(&sa[kk][ar+i]);
