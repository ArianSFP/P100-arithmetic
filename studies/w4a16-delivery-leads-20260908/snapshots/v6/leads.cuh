    ...
    for(int job=blockIdx.x;job<experts*mt*ng;job+=gridDim.x) {
        ...
        auto fetch=[&](int g) {
            ...
            for(int l=0;l<2;++l) {
                ...
                if(MT>=32?l<MT/32:at<MT*4) {
                if constexpr(L==0||L==4) {
                    const int row=tm*MT+at/4;
                    const size_t ai=(size_t(e)*t+row)*k+g*32+8*(at%4);
                    if constexpr(L==0)pa[l]=row<t?*reinterpret_cast<const uint4*>(reinterpret_cast<const half*>(ap)+ai):uint4{};
                    else {
                        pf[2*l]=row<t?*reinterpret_cast<const float4*>(reinterpret_cast<const float*>(ap)+ai):float4{};
                        pf[2*l+1]=row<t?*reinterpret_cast<const float4*>(reinterpret_cast<const float*>(ap)+ai+4):float4{};
                    }
        ...
        auto stage=[&]() {
            ...
            for(int l=0;l<2;++l) {
                ...
                if constexpr(L==0||L==4) {
                    ...
                    const float *vf=reinterpret_cast<const float*>(&pf[2*l]);
                    #pragma unroll
                    for(int i=0;i<8;++i) {
                        if constexpr(L==0)sa[8*(at%4)+i][at/4]=__half2float(v[i]);
                        else sa[8*(at%4)+i][at/4]=__half2float(__float2half_rn(vf[i]));
                    }
                } else {
                    float av[8];
...
inline void launch_delivery(int code,const half *row,const half *tile16,const float *tile32,
                            ...
                            int t,int n,int k,int e,int grid,const float *raw) {
    ...
    #undef TINY
    #define FUSED(U) if(code==4000+U){delivery_lead<4,1,U,2><<<grid,128>>>(raw,wt,out,t,n,k,e);return;}
    FUSED(16) FUSED(32)
    #undef FUSED
    #undef LS
    #undef LD
