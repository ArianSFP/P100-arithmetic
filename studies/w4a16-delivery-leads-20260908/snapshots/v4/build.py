...
def main():
    ...
    s=replace_once(s,'    if(c.kind==-3){', '''    if(c.kind==1){launch_delivery(c.column,a,lead16,lead32,w,leadw,out,t,n,k,e,grid);return;}
    if(c.kind==-3){''')
    s=replace_once(s,'    std::string only;', '    int approved=0,vectorprep=0;\n    std::string only;')
    s=replace_once(s,'        if(key=="--tokens")t=v;', '        if(key=="--gpu-approved")approved=v;else if(key=="--vector-prep")vectorprep=v;else if(key=="--tokens")t=v;')
    ...
    s=replace_once(s,'    da.put(a);dh.put(ha);dw4.put(w4);dw8.put(w8);', '''    da.put(a);dh.put(ha);dw4.put(w4);dw8.put(w8);
    ...
    s=replace_once(s,'auto run=[&](const Config&c,bool prep){if(prep&&c.kind!=-3)prepare_a16<<<std::min<size_t>((ac+255)/256,4096),256>>>(da.p,dh.p,ac);', '''auto run=[&](const Config&c,bool prep){
        if(prep&&c.kind!=-3) {
            ...
            else if(vectorprep)prepare_a16_vector<<<std::min<size_t>((ac/8+255)/256,4096),256>>>(da.p,dh.p,ac);
    ...
    s=replace_once(s,'    cudaDeviceProp prop{};', '    if(vectorprep<0||vectorprep>1)throw std::runtime_error("invalid vector preparation flag");\n    printf("VECTOR_PREP %d\\n",vectorprep);\n    cudaDeviceProp prop{};')
