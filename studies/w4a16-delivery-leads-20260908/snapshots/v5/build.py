...
def main():
    ...
    if(c.kind==-3){''')
    s=replace_once(s,'    std::string only;', '    int approved=0,vectorprep=0,prepgrid=0;\n    std::string only;')
    s=replace_once(s,'        if(key=="--tokens")t=v;', '        if(key=="--gpu-approved")approved=v;else if(key=="--prep-grid")prepgrid=v;else if(key=="--vector-prep")vectorprep=v;else if(key=="--tokens")t=v;')
