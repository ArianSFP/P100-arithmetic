...
template<class T>__global__ void cropout(const T*in,float*out,int M,int N,int MP){int m=blockIdx.x*256+threadIdx.x,n=blockIdx.y;if(m<M&&n<N)out[size_t(n)*M+m]=float(in[size_t(n)*MP+m]);}
int main(int argc,char**argv){
 ...
 CK(cudaMalloc(&dq,q.size()*34));CK(cudaMalloc(&dw,size_t(M+512)*K*2));CK(cudaMalloc(&dx,x.size()*2));CK(cudaMalloc(&di,x.size()*4));CK(cudaMalloc(&dh,size_t(M+512)*N*2));CK(cudaMalloc(&out,size_t(M)*N*4));
 ...
 int pm=0;int pad=0,ldw=M,ldx=N;auto setpad=[&](int v){pad=v;ldw=M+v;ldx=N+v;};
 half *wt,*xt;float*tmp;CK(cudaMalloc(&wt,size_t(M+512)*K*2));CK(cudaMalloc(&xt,size_t(N+512)*K*2));CK(cudaMalloc(&tmp,size_t(M+512)*N*4));
 ...
 auto prepare=[&](int layout){if(layout&1)tw();else pw();if(pm)CK(cudaMemsetAsync(dw+size_t(M)*K,0,size_t(pm)*K*2));if(layout&2)tx();else px();};

 auto output=[&](int mode,int layout){if(layout&4){if(mode)transwide<<<dim3((N+31)/32,(M+31)/32),dim3(32,8)>>>(dh,out,M,N);else trans<<<dim3((N+31)/32,(M+31)/32),dim3(32,8)>>>(tmp,out,M,N);}else if(pm){if(mode)cropout<<<dim3((M+255)/256,N),256>>>(dh,out,M,N,M+pm);else cropout<<<dim3((M+255)/256,N),256>>>(tmp,out,M,N,M+pm);}else if(mode)wide();};
 auto multiply=[&](int mode,int algo,int layout){
  ...
  int lda=swap?(xtrans?ldx:K):(wtrans?ldw:K),ldb=swap?(wtrans?ldw:K):(xtrans?ldx:K),mm=swap?N:M+pm,nn=swap?M+pm:N;
  return cublasGemmEx(b,ta,tb,mm,nn,K,mode?(void*)&ha:(void*)&fa,a,CUDA_R_16F,lda,bb,CUDA_R_16F,ldb,mode?(void*)&hb:(void*)&fb,mode?(void*)dh:((swap||pm)?(void*)tmp:(void*)out),mode?CUDA_R_16F:CUDA_R_32F,mm,mode?CUBLAS_COMPUTE_16F:CUBLAS_COMPUTE_32F,(cublasGemmAlgo_t)algo);
 };
 ...
 for(int v:{0,128,256,512})for(int layout:{2,6})for(int mode=0;mode<2;mode++){
  pm=v;setpad(0);prepare(layout);BL(multiply(mode,mode?3:6,layout));output(mode,layout);CK(cudaDeviceSynchronize());CK(cudaMemcpy(got.data(),out,got.size()*4,cudaMemcpyDeviceToHost));auto&base=mode?href:ref;
  size_t diff=0;for(size_t i=0;i<got.size();i++)diff+=memcmp(&got[i],&base[i],4)!=0;
  printf("{\"type\":\"m_pad_check\",\"mode\":%d,\"layout\":%d,\"pad\":%d,\"different\":%zu}\n",mode,layout,v,diff);
  if(!diff)cs.push_back({mode,layout,v});
 }
 if(argc==5){
  for(int i=0;i<3;i++)for(auto c:cs){pm=c.pad;setpad(0);prepare(c.layout);BL(multiply(c.mode,c.mode?3:6,c.layout));output(c.mode,c.layout);}CK(cudaDeviceSynchronize());
  for(int sample=0;sample<9;sample++)for(size_t j=0;j<cs.size();j++){
   auto c=cs[(j+sample*7)%cs.size()];pm=c.pad;setpad(0);prepare(c.layout);
   ...
   printf("{\"type\":\"m_padding\",\"sample\":%d,\"mode\":%d,\"layout\":%d,\"pad\":%d,\"gemm_us\":%.6f,\"pipeline_us\":%.6f}\n",sample,c.mode,c.layout,c.pad,g,p);
  }
 }
 for(auto c:cs){pm=c.pad;setpad(0);prepare(c.layout);BL(multiply(c.mode,c.mode?3:6,c.layout));output(c.mode,c.layout);CK(cudaDeviceSynchronize());CK(cudaMemcpy(got.data(),out,got.size()*4,cudaMemcpyDeviceToHost));auto&base=c.mode?href:ref;if(memcmp(base.data(),got.data(),got.size()*4))return 10;}
