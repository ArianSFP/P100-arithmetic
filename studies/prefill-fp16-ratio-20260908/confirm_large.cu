...
int main(int argc,char**argv){
 if(argc!=13&&argc!=14&&argc!=15)return 2;int M=atoi(argv[1]),N=atoi(argv[2]),K=atoi(argv[3]);unsigned rng=atoi(argv[4]);
 ...
 std::vector<Q8> q(size_t(M)*K/32);for(auto&g:q){g.d=__float2half_rn((0.003f+float(next()%12001)*0.000001f));for(auto&v:g.q)v=int(next()%256)-128;}
 ...
 struct Config{int mode,algo,layout,pad,arm;};std::vector<Config>cs;
 for(int mode=0;mode<2;mode++){
  cs.push_back({mode,mode?99:-1,0,0,0});
  cs.push_back({mode,atoi(argv[mode?10:9]),0,0,1});
  cs.push_back({mode,atoi(argv[mode?7:5]),atoi(argv[mode?8:6]),atoi(argv[mode?12:11]),2});
 }
 auto verify=[&](){for(auto c:cs){setpad(c.pad);prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);CK(cudaDeviceSynchronize());CK(cudaMemcpy(got.data(),out,got.size()*4,cudaMemcpyDeviceToHost));auto&base=c.mode?href:ref;if(memcmp(base.data(),got.data(),got.size()*4))return false;}return true;};
 if(!verify())return 11;
 printf("{\"type\":\"confirmation_check\",\"configs\":%zu,\"outputs_per_config\":%zu,\"different\":0,\"phase\":\"before\"}\n",cs.size(),got.size());
 if(argc==13){
  for(int i=0;i<5;i++)for(auto c:cs){setpad(c.pad);prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);}CK(cudaDeviceSynchronize());
  for(int sample=0;sample<9;sample++)for(int j=0;j<3;j++){
   int arm=(j+sample)%3;
   for(int leg=0;leg<4;leg++){
    int mode=((leg==0||leg==3)?0:1)^(sample%2);
    auto c=cs[mode*3+arm];
    double p=time([&](){setpad(c.pad);prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);},3);
    printf("{\"type\":\"confirmation\",\"sample\":%d,\"leg\":%d,\"mode\":%d,\"algo\":%d,\"layout\":%d,\"arm\":%d,\"pipeline_us\":%.6f}\n",sample,leg,c.mode,c.algo,c.layout,c.arm,p);
   }
  }
  for(auto c:cs){setpad(c.pad);prepare(c.layout);for(int sample=0;sample<5;sample++){
   double g=time([&](){BL(multiply(c.mode,c.algo,c.layout));},3);
   printf("{\"type\":\"confirmation_gemm\",\"sample\":%d,\"mode\":%d,\"arm\":%d,\"gemm_us\":%.6f}\n",sample,c.mode,c.arm,g);
  }}
 }

 if(argc==15){
  int mode=atoi(argv[13]),arm=atoi(argv[14]);if(mode<0||mode>1||arm<0||arm>2)return 2;auto c=cs[mode*3+arm];setpad(c.pad);
  for(int j=0;j<30;j++){prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);}CK(cudaDeviceSynchronize());
  for(int sample=0;sample<31;sample++){
   ...
   printf("{\"type\":\"isolated\",\"sample\":%d,\"mode\":%d,\"arm\":%d,\"pipeline_us\":%.6f}\n",sample,mode,arm,p);
  }
 }
 if(!verify())return 12;
 printf("{\"type\":\"confirmation_check\",\"configs\":%zu,\"outputs_per_config\":%zu,\"different\":0,\"phase\":\"after\"}\n",cs.size(),got.size());
