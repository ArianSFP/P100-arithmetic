...
run=R/('layout-'+str(time.time_ns()));run.mkdir();p=None
try:
 manifest={x:hashlib.sha256((R/x).read_bytes()).hexdigest() for x in ['layout','layout.cu','run_layout.py']}
   ...
   for n in widths:
    for m,k in [(4352,5120),(5120,4352)]:
     for rep in range(1):
      ...
      cmd=['/usr/bin/env','-i','PATH=/usr/local/cuda-12.8/bin:/usr/bin:/bin','LD_LIBRARY_PATH=/usr/local/cuda-12.8/lib64','CUDA_DEVICE_ORDER=PCI_BUS_ID','CUDA_VISIBLE_DEVICES='+UUID,'/usr/bin/taskset','--cpu-list','0-11',str(R/'layout'),str(m),str(n),str(k),str(802110+rep)]
      if sanitizer:
       idx=cmd.index(str(R/'layout'));cmd[idx:idx]=['/usr/local/cuda-12.8/bin/compute-sanitizer','--tool','memcheck','--error-exitcode','9'];cmd.append('--check-only')
