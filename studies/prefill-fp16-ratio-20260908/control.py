...
TOKEN='prefill-fp16-large-'+str(time.time_ns())+'-gpu1'
...
while True:
 try:fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB);break
  ...
  time.sleep(1)
guard();h=health();append('HELD: '+TOKEN+'; GPU1-only2048+ FP16 ratio, shared lock held through bounded tests/build gaps. GPU0/2/3 untouched. No production edits.')
run=R/('large-'+str(time.time_ns()));run.mkdir();save(run/'hold.json',dict(token=TOKEN,before=h,pid=os.getpid(),controller_sha256=sha(Path(__file__))))
...
try:
 with (run/'telemetry.csv').open('w') as tf:
  ...
  while time.monotonic()<end:
   ...
   if not tag.replace('-','').isalnum() or name not in ['layout_fused','layout_native','confirm_large']:raise RuntimeError('Unknown worker/tag')
   ...
   before=health();guard();meta=dict(command=cmd,before=before,binary_sha256=sha(binary),source_sha256=sha(source),token=TOKEN)
