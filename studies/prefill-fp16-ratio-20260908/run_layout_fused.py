...
wait_end=time.monotonic()+900
while True:
 try:fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB);break
 except BlockingIOError:
  if time.monotonic()>wait_end:raise RuntimeError('Benchmark lock still held after 900s; no GPU worker started')
  time.sleep(1)

guard();before=health()
append('HELD: '+TOKEN+'; GPU1 only, stage timing and cuBLAS sweep. Global lock held; GPU2/3 untouched. No production change.')
run=R/('layout-fused-'+str(time.time_ns()));run.mkdir();p=None
try:
 manifest={x:hashlib.sha256((R/x).read_bytes()).hexdigest() for x in ['layout_fused','layout_fused.cu','run_layout_fused.py']}
 ...
 with (run/'telemetry.csv').open('w') as tf:
  ...
  try:
   sanitizer='--memcheck' in sys.argv
   widths=[int(x) for x in sys.argv[1:] if x!='--memcheck'] or [256]
   for n in widths:
    for m,k in [(4352,5120),(5120,4352)]:
     for rep in range(1):
      guard();h=health();tag=f'{m}-{n}-{k}-{rep}'
      cmd=['/usr/bin/env','-i','PATH=/usr/local/cuda-12.8/bin:/usr/bin:/bin','LD_LIBRARY_PATH=/usr/local/cuda-12.8/lib64','CUDA_DEVICE_ORDER=PCI_BUS_ID','CUDA_VISIBLE_DEVICES='+UUID,'/usr/bin/taskset','--cpu-list','0-11',str(R/'layout_fused'),str(m),str(n),str(k),str(802110+rep)]
      if sanitizer:
       idx=cmd.index(str(R/'layout_fused'));cmd[idx:idx]=['/usr/local/cuda-12.8/bin/compute-sanitizer','--tool','memcheck','--error-exitcode','9'];cmd.append('--check-only')
