...
int main(int argc,char**argv){
 ...
 auto tw=[&](){trans<<<dim3((K+31)/32,(M+31)/32),dim3(32,8)>>>(dw,wt,M,K);};
 auto tx=[&](){trans<<<dim3((K+31)/32,(N+31)/32),dim3(32,8)>>>(dx,xt,N,K);};
 auto prepare=[&](int layout){pw();px();if(layout&1)tw();if(layout&2)tx();};
 ...
 for(int layout=0;layout<8;layout++){
  prepare(layout);
  for(int mode=0;mode<2;mode++)for(int a=-1;a<7;a++){
 ...
 if(argc==5){
  ...
  for(int s=0;s<7;s++)for(size_t j=0;j<cs.size();j++){
   auto c=cs[(j+s*7)%cs.size()];prepare(c.layout);
   double g=time([&](){BL(multiply(c.mode,c.algo,c.layout));},3);
   double p=time([&](){prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);},3);
