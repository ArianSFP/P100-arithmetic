...
__global__ void prepwt(const Q8*q,half*out,int M,int K){
 ...
 #pragma unroll
 for(int j=0;j<32;j+=8)if(m<M&&k+j<K)out[size_t(k+j)*M+m]=t[threadIdx.x][threadIdx.y+j];
}
__global__ void prepxt(const float*in,half*out,int N,int K){
 ...
 #pragma unroll
 for(int j=0;j<32;j+=8)if(n<N&&k+j<K)out[size_t(k+j)*N+n]=t[threadIdx.x][threadIdx.y+j];
}
...
int main(int argc,char**argv){
 ...
 half *wt,*xt;float*tmp;CK(cudaMalloc(&wt,size_t(M)*K*2));CK(cudaMalloc(&xt,size_t(N)*K*2));CK(cudaMalloc(&tmp,size_t(M)*N*4));
 auto pw=[&](){prep_native<<<M*K/2048,32>>>(dq,dw);};
 auto px=[&](){prepx<<<(N*K+255)/256,256>>>(di,dx,N*K);};
 auto tw=[&](){prepwt<<<dim3((K+31)/32,(M+31)/32),dim3(32,8)>>>(dq,wt,M,K);};
 auto tx=[&](){prepxt<<<dim3((K+31)/32,(N+31)/32),dim3(32,8)>>>(di,xt,N,K);};
 ...
 auto multiply=[&](int mode,int algo,int layout){
  ...
  int lda=swap?(xtrans?N:K):(wtrans?M:K),ldb=swap?(wtrans?M:K):(xtrans?N:K),mm=swap?N:M,nn=swap?M:N;
 ...
 struct Config{int mode,algo,layout;};std::vector<Config>cs;
 for(int layout=0;layout<8;layout++){
  prepare(layout);
  for(int mode=0;mode<2;mode++)for(int a=-1;a<24;a++){
   if(argc==6&&(a!=-1||(layout!=0&&layout!=3&&layout!=7)))continue;
   int algo=a==-1?(mode?99:-1):a;auto status=multiply(mode,algo,layout);
   if(status==CUBLAS_STATUS_NOT_SUPPORTED){printf("{\"type\":\"unsupported\",\"mode\":%d,\"algo\":%d,\"layout\":%d}\n",mode,algo,layout);continue;}BL(status);output(mode,layout);CK(cudaDeviceSynchronize());CK(cudaMemcpy(got.data(),out,got.size()*4,cudaMemcpyDeviceToHost));auto&base=mode?href:ref;
   size_t diff=0;double e=0,r=0;for(size_t i=0;i<got.size();i++){if(!std::isfinite(got[i]))return 8;diff+=memcmp(&got[i],&base[i],4)!=0;double d=double(got[i])-ref[i];e+=d*d;r+=double(ref[i])*ref[i];}
   printf("{\"type\":\"layout_check\",\"mode\":%d,\"algo\":%d,\"layout\":%d,\"different\":%zu,\"relL2\":%.9g}\n",mode,algo,layout,diff,sqrt(e/r));
   if(!diff)cs.push_back({mode,algo,layout});
  }
 }
 if(argc==5){
  for(int i=0;i<3;i++)for(auto c:cs){prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);}CK(cudaDeviceSynchronize());
  for(int s=0;s<5;s++)for(size_t j=0;j<cs.size();j++){
   auto c=cs[(j+s*7)%cs.size()];prepare(c.layout);
   double g=time([&](){BL(multiply(c.mode,c.algo,c.layout));},2);
   double p=time([&](){prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);},2);
   printf("{\"type\":\"layout\",\"sample\":%d,\"mode\":%d,\"algo\":%d,\"layout\":%d,\"gemm_us\":%.6f,\"pipeline_us\":%.6f}\n",s,c.mode,c.algo,c.layout,g,p);
  }
 }

 for(auto c:cs){prepare(c.layout);BL(multiply(c.mode,c.algo,c.layout));output(c.mode,c.layout);CK(cudaDeviceSynchronize());CK(cudaMemcpy(got.data(),out,got.size()*4,cudaMemcpyDeviceToHost));auto&base=c.mode?href:ref;if(memcmp(base.data(),got.data(),got.size()*4))return 10;}
