...
// Q4 catalog pointers carry a low-bit format tag. Tensor storage stays native,
// so ordinary decode and tensor readback use the unchanged GGML Q4 paths.
...
static void aw_q8_service_m32(
    ...
    for (int work_id = blockIdx.x; work_id < n_work; work_id += gridDim.x) {
        ...
        float pd0, pd1; float q4b0[8], q4b1[8];
        ...
        auto fetchb = [&](int stage) {
            if (aw_q4_tag(desc.weight)) {
                if ((uintptr_t)desc.weight & 4u) {
                    const int md=aw_q4_tag(desc.weight)==1?2:4;
                    const uint2 raw0=__ldg((const uint2*)(aw_q4_stage(desc.weight,col0+b_r0,stage,k_stride)+md*64+(b_r0%64)*16+(b_k&15)));
                    const uint2 raw1=__ldg((const uint2*)(aw_q4_stage(desc.weight,col0+b_r1,stage,k_stride)+md*64+(b_r1%64)*16+(b_k&15)));
                    const float d0=aw_q4_scale(desc.weight,col0+b_r0,stage,k_stride),d1=aw_q4_scale(desc.weight,col0+b_r1,stage,k_stride);
                    const float m0=aw_q4_offset(desc.weight,col0+b_r0,stage,k_stride),m1=aw_q4_offset(desc.weight,col0+b_r1,stage,k_stride);
                    #pragma unroll
                    for(int j=0;j<8;++j) {
                        int idx=j;
                        q4b0[j]=aw_q4_decode_byte((j<4?raw0.x:raw0.y),idx%4,b_k/16,aw_q4_tag(desc.weight),d0,m0);
                        q4b1[j]=aw_q4_decode_byte((j<4?raw1.x:raw1.y),idx%4,b_k/16,aw_q4_tag(desc.weight),d1,m1);
                    }
                } else {
                    #pragma unroll
                    for (int j=0;j<8;++j) {
                        q4b0[j]=aw_q4_value(desc.weight,col0+b_r0,stage,b_k+j,k_stride);
                        q4b1[j]=aw_q4_value(desc.weight,col0+b_r1,stage,b_k+j,k_stride);
                    }
                }
                return;
            }
        ...
        auto stage_a_part = [&](int buffer, int i) {
            ...
            const float * a0 = (const float *) &pa0;
            const float * a1 = (const float *) &pa1;
            fA[(a_k + i    )*32 + aoff] = (aw_q4_tag(desc.weight) ? __half2float(__float2half_rn(a0[i])) : a0[i]);
            fA[(a_k + i + 4)*32 + aoff] = (aw_q4_tag(desc.weight) ? __half2float(__float2half_rn(a1[i])) : a1[i]);
        };
        auto stage_b_part = [&](int buffer, int i) {
            float * fB = (float *) sm.stage.B[buffer];
            if (aw_q4_tag(desc.weight)) {
                fB[(b_k+2*i)*64+boff0]=q4b0[2*i];
                fB[(b_k+2*i+1)*64+boff0]=q4b0[2*i+1];
                fB[(b_k+2*i)*64+boff1]=q4b1[2*i];
                fB[(b_k+2*i+1)*64+boff1]=q4b1[2*i+1];
                return;
            }
...
static void aw_q8_service_m16_rail_pair(
    ...
    for (int work_id = blockIdx.x;
        ...
        auto fetch_q = [&](const char * tile_stage,
            ...
            if (aw_q4_tag(desc.weight)) {
                if((uintptr_t)desc.weight & 4u) {
                    // Existing weight_stage advances in Q8-sized logical stages.
                    const int stage=(tile_stage-weight_stage)/AW_T64_STAGE_BYTES+q4_current_stage;
                    q0=aw_q4_load16(desc.weight,col0+b_r0,stage,k);
                    q1=aw_q4_load16(desc.weight,col0+b_r1,stage,k);
                    scale0=aw_q4_scale(desc.weight,col0+b_r0,stage,k);
                    scale1=aw_q4_scale(desc.weight,col0+b_r1,stage,k);
                } else {q0={};q1={};scale0=scale1=0;}
                return;
            }
            scale0 = __half2float(
                    *(const half *)
        ...
        for (int stage = 0;
            ...
            for (int i = 0; i < 4; ++i) {
                sm.group[group].A[kgrp]
                        [a_k + i][a_r] =
                        (aw_q4_tag(desc.weight) ? __half2float(__float2half_rn(a0[i])) : a0[i]);
                sm.group[group].A[kgrp]
                        [a_k + i + 4][a_r] =
                        (aw_q4_tag(desc.weight) ? __half2float(__float2half_rn(a1[i])) : a1[i]);
            }
            __syncwarp();
            ...
            for (int word = 0; word < 4; ++word) {
                ...
                for (int byte = 0;
                    ...
                    const int kk =
                            word*4 + byte;
                    const float bv0 = ((uintptr_t)desc.weight & 4u) ? aw_q4_decode_byte(packed0,byte,kgrp,aw_q4_tag(desc.weight),scale0,aw_q4_offset(desc.weight,col0+b_r0,stage,k)) : aw_q4_tag(desc.weight) ? aw_q4_value(desc.weight,col0+b_r0,stage,kgrp*16+kk,k) : __fmul_rn(
                                (float)
                                (signed char)
                                ((packed0 >>
                                  (byte*8)) & 0xffu),
                                scale0);
                    const float bv1 = ((uintptr_t)desc.weight & 4u) ? aw_q4_decode_byte(packed1,byte,kgrp,aw_q4_tag(desc.weight),scale1,aw_q4_offset(desc.weight,col0+b_r1,stage,k)) : aw_q4_tag(desc.weight) ? aw_q4_value(desc.weight,col0+b_r1,stage,kgrp*16+kk,k) : __fmul_rn(
                                (float)
                                (signed char)
                                ((packed1 >>
                                  (byte*8)) & 0xffu),
            ...
            auto stage_a = [&](int buffer,
                ...
                for (int i = 0; i < 4; ++i) {
                    sm.stage.A[buffer][group]
                            [a_k + i][a_r] = (aw_q4_tag(desc.weight) ? __half2float(__float2half_rn(a0[i])) : a0[i]);
                    sm.stage.A[buffer][group]
                            [a_k + i + 4][a_r] =
                            (aw_q4_tag(desc.weight) ? __half2float(__float2half_rn(a1[i])) : a1[i]);
                }
            ...
            for (int stage = 0;
                ...
                const char * tile_stage =
                        ...
                        ((size_t)
                            ...
                            n_kblocks + stage)*
                            AW_T64_STAGE_BYTES;
                if (aw_q4_tag(first_desc.weight)) {
                    if (stage >= 2) aw_cohortrail_sync<CTA_THREADS>(2+buffer);
                    for (int row=producer_tid;row<64;row+=PRODUCER_THREADS) {
                        if((uintptr_t)first_desc.weight & 4u) {
                            uint4 raw=aw_q4_load16(first_desc.weight,col0+row,stage,k_stride);
                            const uint32_t words[4]={raw.x,raw.y,raw.z,raw.w};
                            float d=aw_q4_scale(first_desc.weight,col0+row,stage,k_stride);
                            float m=aw_q4_offset(first_desc.weight,col0+row,stage,k_stride);
                            #pragma unroll
                            for(int ki=0;ki<32;++ki)
                                sm.stage.B[buffer][ki][row]=aw_q4_decode_byte(words[(ki&15)/4],ki%4,ki/16,aw_q4_tag(first_desc.weight),d,m);
                        } else {
                            #pragma unroll
                            for (int ki=0;ki<32;++ki)
                                sm.stage.B[buffer][ki][row]=aw_q4_value(first_desc.weight,col0+row,stage,ki,k_stride);
                        }
                    }
                } else if constexpr (PRODUCER_THREADS == 64) {
                    const float scale =
                            __half2float(
                                *(const half *)
                                (tile_stage +
...
// Packed T64: 64 scales, optional 64 offsets, then 16 planes of 64 packed bytes.
// Its byte count equals native Q4; the transform changes only addresses.
__global__ static void aw_repack_q4_t64(const char * src, char * dst,
        size_t blocks, int n, int k, int tag, bool inverse=false) {
    const int md=tag==1?2:4, kb=k/32;
    for(size_t b=(size_t)blockIdx.x*blockDim.x+threadIdx.x;b<blocks;b+=(size_t)gridDim.x*blockDim.x) {
        const size_t tile_index=b/64, tiles_per_matrix=(size_t)(n/64)*kb;
        const size_t matrix=tile_index/tiles_per_matrix, within=tile_index%tiles_per_matrix;
        const int lane=b%64, row=(within/kb)*64+lane, stage=within%kb;
        const size_t native=((matrix*n+row)*kb+stage)*(md+16), tile=tile_index*64*(md+16);
        for(int m=0;m<md/2;++m) {
            size_t x=native+2*m,y=tile+m*128+lane*2;
            *(uint16_t*)(dst+(inverse?x:y))=*(const uint16_t*)(src+(inverse?y:x));
        }
        if(!inverse) {
            uint32_t words[4];
            #pragma unroll
            for(int j=0;j<4;++j) {
                const uint16_t lo=*(const uint16_t*)(src+native+md+j*4);
                const uint16_t hi=*(const uint16_t*)(src+native+md+j*4+2);
                words[j]=uint32_t(lo)|(uint32_t(hi)<<16);
            }
            *(uint4*)(dst+tile+md*64+lane*16)=make_uint4(words[0],words[1],words[2],words[3]);
        } else {
            #pragma unroll
            for(int i=0;i<16;++i)dst[native+md+i]=src[tile+md*64+lane*16+i];
        }
    }
}
...
__global__ static void aw_live_build_plan(
    ...
    if (blockDim.x != 256 ||
        ...
        for (int desc = 0; desc < descriptors; ++desc) {
            ...
            gate_desc[desc] = {
                input, aw_expert_ptr(weight_ptrs[cell*3 + AW_PROJECTION_GATE], expert, q8_gate_bytes),
                gate + (size_t) row0*gate_up_stride,
                route_input + row0, row0, rows, cell, expert
            };
            up_desc[desc] = {
                input, aw_expert_ptr(weight_ptrs[cell*3 + AW_PROJECTION_UP], expert, q8_gate_bytes),
                up + (size_t) row0*gate_up_stride,
                route_input + row0, row0, rows, cell, expert
            };
            down_desc[desc] = {
                middle + (size_t) row0*AW_EXPERT_FF,
                aw_expert_ptr(weight_ptrs[cell*3 + AW_PROJECTION_DOWN], expert, q8_down_bytes),
    ...
    gate_desc[tid] = {
        input,
        aw_expert_ptr(weight_ptrs[cell*3 + AW_PROJECTION_GATE], expert, q8_gate_bytes),
        gate + (size_t) row0*gate_up_stride,
        route_input + row0, row0, rows, cell, expert
    };
    up_desc[tid] = {
        input,
        aw_expert_ptr(weight_ptrs[cell*3 + AW_PROJECTION_UP], expert, q8_gate_bytes),
        up + (size_t) row0*gate_up_stride,
        route_input + row0, row0, rows, cell, expert
    };
    down_desc[tid] = {
...
static std::array<std::array<aw_q4_cache_slot,12>,AW_GPU_COUNT> aw_q4_cache;
static size_t aw_q4_cache_bytes[AW_GPU_COUNT]={};
static void aw_q4_cache_forget(const void * data,size_t bytes) {
    uintptr_t lo=(uintptr_t)data,hi=lo+bytes;
    for(auto & dev:aw_q4_cache)for(auto & slot:dev) {
        uintptr_t ptr=(uintptr_t)slot.source & ~(uintptr_t)7;
        if(ptr>=lo && ptr<hi)slot.source=nullptr;
    }
}

static const char * aw_q4_cache_get(const aw_layout_entry & e, cudaStream_t stream, int call) {
    const char * source=(const char*)e.data;
    const int tag=aw_q4_tag(source);
    if(!tag)return source;
    CUDA_CHECK(cudaSetDevice(e.device));
    auto & slots=aw_q4_cache[e.device];
    aw_q4_cache_slot * slot=nullptr;
    for(auto & x:slots)if(x.source==e.data){slot=&x;break;}
    bool hit=slot!=nullptr;
    if(!slot)for(auto & x:slots)if(x.used_call!=call && (!slot || x.used_call<slot->used_call))slot=&x;
    GGML_ASSERT(slot);
    if(slot->done)CUDA_CHECK(cudaStreamWaitEvent(stream,slot->done,0));
    if(!hit) {
        if(slot->capacity<e.bytes) {
            if(slot->done)CUDA_CHECK(cudaEventSynchronize(slot->done));
            if(slot->data)CUDA_CHECK(cudaFree(slot->data));
            aw_q4_cache_bytes[e.device]-=slot->capacity;
            CUDA_CHECK(cudaMalloc(&slot->data,e.bytes));slot->capacity=e.bytes;
            aw_q4_cache_bytes[e.device]+=e.bytes;
            fprintf(stderr,"Q4 T64 bounded cache GPU%d bytes=%zu\n",e.device,aw_q4_cache_bytes[e.device]);
        }
        aw_repack_q4_t64<<<aw_grid(e.bytes/(tag==1?18:20)),256,0,stream>>>(
                (const char*)((uintptr_t)e.data & ~(uintptr_t)7),(char*)slot->data,
                e.bytes/(tag==1?18:20),e.n,e.k,tag);
        CUDA_CHECK(cudaGetLastError());slot->source=e.data;
    }
    slot->used_call=call;
    return (const char*)((uintptr_t)slot->data|tag|4u);
}
static void aw_q4_cache_finish(const std::array<cudaStream_t,AW_GPU_COUNT> & streams,int call) {
    for(int dev=0;dev<AW_GPU_COUNT;++dev) {
        CUDA_CHECK(cudaSetDevice(dev));
        for(auto & slot:aw_q4_cache[dev])if(slot.used_call==call) {
            if(!slot.done)CUDA_CHECK(cudaEventCreateWithFlags(&slot.done,cudaEventDisableTiming));
            CUDA_CHECK(cudaEventRecord(slot.done,streams[dev]));
        }
    }
}
    ...
    try {
                ...
                for (int owner = 0; owner < AW_GPU_COUNT; ++owner) {
                    for (int group_cell = 0; group_cell < group_cells; ++group_cell) {
                        ...
                        for (int projection = 0; projection < 3; ++projection) {
                            ...
                            weight_ptrs[group][owner][group_cell*3 + projection] = diagonal_panel ? aw_q4_cache_get(*it, streams[owner], live_call) : (const char *)it->data;
                        }
                        if (shared_service) {
                            for (int projection = 0; projection < 3; ++projection) {
                                const auto it = std::find_if(aw_layout_entries.begin(), aw_layout_entries.end(),
        ...
        if (diagonal_panel) {
            aw_live_diagonal_panel(
                    ...
                    diagonal_panel_n);
            aw_q4_cache_finish(streams,live_call);
            if (live_call == 0) {
                fprintf(stderr,
...
void ggml_cuda_affinity_wave_forget_tensor(const ggml_tensor * tensor) {
    ...
    std::lock_guard<std::mutex> lock(aw_layout_mutex);
    aw_q4_cache_forget(tensor->data,ggml_nbytes(tensor));
    aw_t64_tensors.erase(tensor->data);
    aw_t64_tensors.erase((void *)((uintptr_t)tensor->data | 1u));
    aw_t64_tensors.erase((void *)((uintptr_t)tensor->data | 2u));
    aw_layout_entries.erase(std::remove_if(aw_layout_entries.begin(), aw_layout_entries.end(),
            [&](const aw_layout_entry & entry) { return ((uintptr_t)entry.data & ~(uintptr_t)3) == (uintptr_t)tensor->data; }), aw_layout_entries.end());
}
...
static void aw_repack_uploaded_tensor(
    ...
    if (upload_complete && tensor && tensor->data &&
            (tensor->type == GGML_TYPE_Q4_0 || tensor->type == GGML_TYPE_Q4_1) &&
            ggml_cuda_affinity_wave_t64_enabled() && strstr(tensor->name, "_exps") &&
            ggml_is_contiguous(tensor) && tensor->ne[0] % 32 == 0 && tensor->ne[1] % 64 == 0) {
        GGML_ASSERT(aw_q8_cohortrail_enabled());
        int device = 0, layer = -1;
        CUDA_CHECK(cudaGetDevice(&device));
        (void) sscanf(tensor->name, "blk.%d.", &layer);
        const int projection = aw_tensor_projection(tensor);
        if (layer < 0 || layer >= AW_LAYERS || projection < 0) return;
        void * tagged = (void *) ((uintptr_t)tensor->data |
                (tensor->type == GGML_TYPE_Q4_0 ? 1u : 2u));
        std::lock_guard<std::mutex> lock(aw_layout_mutex);
        if (aw_t64_tensors.insert(tagged).second) {
            aw_layout_entries.push_back({tagged, ggml_nbytes(tensor),
                    (int)tensor->ne[1], (int)tensor->ne[0], device, layer, projection});
            fprintf(stderr, "AffinityWave: native Q4 catalog GPU%d layer=%d projection=%d type=%s\n",
                    device, layer, projection, ggml_type_name(tensor->type));
        }
        return;
    }
...
void ggml_cuda_affinity_wave_normalize_decode(void * stream_ptr) {
    ...
    std::vector<aw_layout_entry> entries;
    {
        ...
        for (const aw_layout_entry & entry : aw_layout_entries) {
            if (entry.device == device && aw_q4_tag((const char *)entry.data) == 0 && aw_t64_tensors.count(entry.data) != 0) {
...
extern "C" int aw_q4_semantic_test();
extern "C" int aw_q4_semantic_test() {
    CUDA_CHECK(cudaSetDevice(0));
    size_t checked=0;
    for (int k : {512,2048}) for (int tag : {1,2}) {
        const int n=128, rows=64;
        size_t blocks=(size_t)n*k/32;
        std::vector<unsigned char> q4(blocks*(tag==1?18:20)), q8(blocks*34);
        for (size_t b=0;b<blocks;++b) {
            const float d=std::ldexp(1.0f,-4-(int)(b%5));
            const half hd=__float2half(d), hm=__float2half(-d*8);
            memcpy(q4.data()+b*(tag==1?18:20),&hd,2);
            if(tag==2)memcpy(q4.data()+b*20+2,&hm,2);
            memcpy(q8.data()+b*34,&hd,2);
            for(int i=0;i<16;++i) {
                unsigned q0=(b*7+i*3)%16,q1=(b*11+i*5+1)%16;
                q4[b*(tag==1?18:20)+(tag==1?2:4)+i]=q0|(q1<<4);
                q8[b*34+2+i]=(unsigned char)(int(q0)-8);
                q8[b*34+18+i]=(unsigned char)(int(q1)-8);
            }
        }
        std::vector<float> input(rows*k);
        for(size_t i=0;i<input.size();++i)input[i]=float(int((i*13+i/17)%61)-30)/64;
        aw_device_buffer a(input.size()*4),w4(q4.size()),w8(q8.size()),packed(q8.size());
        aw_device_buffer q4packed(q4.size()), q4restored(q4.size());
        aw_device_buffer out(rows*n*4),descs(4*sizeof(aw_work_desc)),tiles(4*sizeof(aw_tile_desc)),coh(sizeof(aw_m16_cohort));
        CUDA_CHECK(cudaMemcpy(a.get(),input.data(),input.size()*4,cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(w4.get(),q4.data(),q4.size(),cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(w8.get(),q8.data(),q8.size(),cudaMemcpyHostToDevice));
        aw_repack_q8_t64<<<aw_grid(blocks),256>>>((char*)w8.get(),(char*)packed.get(),blocks,n,k);
        aw_repack_q4_t64<<<aw_grid(blocks),256>>>((char*)w4.get(),(char*)q4packed.get(),blocks,n,k,tag);
        aw_repack_q4_t64<<<aw_grid(blocks),256>>>((char*)q4packed.get(),(char*)q4restored.get(),blocks,n,k,tag,true);
        std::vector<unsigned char> restored(q4.size());
        CUDA_CHECK(cudaMemcpy(restored.data(),q4restored.get(),q4.size(),cudaMemcpyDeviceToHost));
        if(restored!=q4){fprintf(stderr,"Q4 T64 roundtrip FAIL\n");return 2;}
        aw_m16_cohort hc={{0,1,2,3}};
        CUDA_CHECK(cudaMemcpy(coh.get(),&hc,sizeof(hc),cudaMemcpyHostToDevice));
        for(int mode=0;mode<4;++mode) {
            std::vector<float> results[2];
            for(int arm=0;arm<2;++arm) {
                aw_work_desc hd[4];aw_tile_desc ht[4];
                const char * weight=arm?(const char*)((uintptr_t)q4packed.get()|tag|4u):(const char*)packed.get();
                for(int i=0;i<4;++i) {
                    hd[i]={a.get(),weight,(float*)out.get(),nullptr,0,rows,0,0};
                    ht[i]={i,i*16,16};
                }
                if(mode==0)ht[0]={0,0,64};
                if(mode==1)ht[0]={0,0,32};
                CUDA_CHECK(cudaMemset(out.get(),0,rows*n*4));
                CUDA_CHECK(cudaMemcpy(descs.get(),hd,sizeof(hd),cudaMemcpyHostToDevice));
                CUDA_CHECK(cudaMemcpy(tiles.get(),ht,sizeof(ht),cudaMemcpyHostToDevice));
                if(mode==0)aw_q8_service_m64_n128_halfpipe_sync<false><<<1,256>>>((aw_work_desc*)descs.get(),(aw_tile_desc*)tiles.get(),1,n,k);
                if(mode==1)aw_q8_service_m32<false,true,true><<<2,128>>>((aw_work_desc*)descs.get(),(aw_tile_desc*)tiles.get(),1,n,k);
                if(mode==2)aw_q8_service_m16_rail_pair<false><<<2,128>>>((aw_work_desc*)descs.get(),(aw_tile_desc*)tiles.get(),1,n,k);
                if(mode==3)aw_q8_service_m16_cohortrail<false,4,64><<<2,320>>>((aw_work_desc*)descs.get(),(aw_tile_desc*)tiles.get(),(aw_m16_cohort*)coh.get(),1,n,k);
                CUDA_CHECK(cudaGetLastError());CUDA_CHECK(cudaDeviceSynchronize());
                results[arm].resize(rows*n);
                CUDA_CHECK(cudaMemcpy(results[arm].data(),out.get(),rows*n*4,cudaMemcpyDeviceToHost));
            }
            if(memcmp(results[0].data(),results[1].data(),rows*n*4)) {
                size_t bad=0;float maxerr=0;
                for(size_t i=0;i<results[0].size();++i)if(results[0][i]!=results[1][i]){++bad;maxerr=std::max(maxerr,std::abs(results[0][i]-results[1][i]));}
                fprintf(stderr,"Q4 FAIL k=%d tag=%d mode=%d bad=%zu maxerr=%g\n",k,tag,mode,bad,maxerr);return 1;
            }
            checked+=rows*n;
            fprintf(stderr,"Q4 PASS k=%d tag=%d mode=%d\n",k,tag,mode);
        }
    }
    fprintf(stderr,"Q4 semantic equality: %zu output words\n",checked);return 0;
}
