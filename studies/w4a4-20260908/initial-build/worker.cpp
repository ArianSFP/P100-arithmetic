...
std::vector<Config> configs(){std::vector<Config>v;for(int h:{0,1})for(int r:{1,2,4})for(int b:{1,4})v.push_back({h,r,b});return v;}
struct Data {
 unsigned m,k,n,g,padded;
 std::vector<uint16_t> a,sc;
 std::vector<unsigned>w,aq;
 std::vector<float>ad,ref;
 Buffer dw,ds,din,daq,dad,partial,out;
 Data(unsigned rows,unsigned cols,unsigned batches,int family):m(rows),k(cols),n(batches),g(k/32),padded((m+31)/32*32),
 a(n*k),sc(size_t(padded)*g),w(size_t(padded)*g*4),aq(n*g*4),ad(n*g),ref(size_t(n)*m),
 dw(w.size()*4),ds(sc.size()*2),din(a.size()*2),daq(aq.size()*4),dad(ad.size()*4),partial(size_t(n)*m*32*4),out(ref.size()*4) {
 ...
 dw.upload(w);ds.upload(sc);din.upload(a);daq.upload(aq);dad.upload(ad);
 ...
 void reference(bool endpoint=false) {
  if(endpoint){
   ...
   daq.upload(aq); // Independent packed endpoint gate, bypass quantizer.
  }
 ...
 void quant(CUmodule mod) {void *args[]={&din.p,&daq.p,&dad.p,&g,&n};launch(fn(mod,"quantize"),(g*n+3)/4,1,1,args);}
 ...
 void run(CUmodule mod,Config c,bool prep) {
  if(prep)quant(mod);
  void *args[]={&dw.p,&ds.p,&daq.p,&dad.p,&partial.p,&m,&g,&n};
...
int main(int argc,char**argv) {try {
 ...
 if(std::string(argv[2])=="check") {
  ...
  for(int round=0;round<9;round++) {
   ...
   for(int j=0;j<13;j++) {int idx=(j+round)%13;if(idx==12)printf("TIME round=%d config=sealed_a16 us=%.6f\n",round,timed(a16,12));
