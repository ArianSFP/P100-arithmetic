...
void launch(CUfunction f,unsigned x,unsigned y,unsigned z,void **args,unsigned threads=128,unsigned shared=0){CK(cuLaunchKernel(f,x,y,z,threads,1,1,shared,0,args,0));}
struct Config {int h,r,b,nw=4;bool fuse=false;std::string name()const{return "w4a4_h"+std::to_string(h)+"_r"+std::to_string(r)+"_b"+std::to_string(b)+(nw==4 ? "" : "_w"+std::to_string(nw))+(fuse ? "_f":"");}};
std::vector<Config> configs(){std::vector<Config>v;for(int h:{0,1,2})for(int r:{1,2,4})for(int b:{1,4})v.push_back({h,r,b});
 for(auto c:std::vector<Config>{{2,8,1},{2,8,4},{2,16,1}})v.push_back(c);
 for(auto c:std::vector<Config>{{2,2,1},{2,4,1},{2,8,1},{2,1,4},{2,2,4},{2,4,4}})for(int nw:{1,2,8}){c.nw=nw;v.push_back(c);}
 for(int h:{0,2}) {
  for(int r:{1,2})for(int nw:{8,16,32})v.push_back({h,r,1,nw,true});
  for(int nw:{8,16})v.push_back({h,1,4,nw,true});
 }
 return v;}
struct Data {
 ...
 for(size_t i=0;i<a.size();i++) {
  ...
  if(family==4){unsigned bits=i&65535;a[i]=((bits>>10)&31)==31 ? 0:uint16_t(bits);}
 }
 ...
 void reference(bool endpoint=false) {
  if(endpoint){
   for(size_t i=0;i<aq.size();i++)aq[i]=(i/4)%3==0 ? 0x88888888u : (i/4)%3==1 ? 0x77777777u : 0x78787878u;
 ...
 void quant(CUmodule mod,int mode=2) {void *args[]={&din.p,&daq.p,&dap.p,&dad.p,&g,&n};launch(fn(mod,mode==0 ? "quantize_nibbles" : mode==1 ? "quantize_planes" : "quantize"),(g*n+3)/4,1,1,args);}
 ...
 void run(CUmodule mod,Config c,bool prep) {
  if(prep)quant(mod,c.h==2 ? 1:0);
  CUdeviceptr weights=c.h==2 ? dwp.p:dw.p,acts=c.h==2 ? dap.p:daq.p;
  CUdeviceptr dst=c.fuse ? out.p:partial.p;
  void *args[]={&weights,&ds.p,&acts,&dad.p,&dst,&m,&g,&n};
  unsigned tile=c.fuse ? 32*c.r:32*c.nw*c.r;
  launch(fn(mod,c.name().c_str()),(m+tile-1)/tile,(n+c.b-1)/c.b,c.fuse ? 1:32,args,32*c.nw,c.fuse ? 32*c.b*c.r*32*4:0);
  if(!c.fuse){void *red[]={&partial.p,&out.p,&m};launch(fn(mod,"reduce32"),(m+127)/128,n,1,red);}
 }
 void validate(CUmodule mod,Config c,bool prep=false) {
  run(mod,c,prep);std::vector<float> got(ref.size());CK(cuMemcpyDtoH(got.data(),out.p,got.size()*4));
...
int main(int argc,char**argv) {try {
 require((argc==3 && std::string(argv[2])=="check") || (argc==6 && std::string(argv[2])=="bench") || (argc==7 && std::string(argv[2])=="frozen"),"usage: worker kernels.cubin check | bench M K N | frozen M K N configs.txt");
 ...
 if(std::string(argv[2])=="check") {
  for(int family=0;family<4;family++)for(auto shape:std::vector<std::vector<unsigned>>{{33,96,1},{129,32,3},{513,544,4}}) {
   Data d(shape[0],shape[1],shape[2],family);d.check_quant(mod);d.reference();for(auto c:configs())d.validate(mod,c,true);
   if(family==0){d.reference(true);for(auto c:configs())d.validate(mod,c);}
  }
  Data exhaustive(16,65536,1,4);exhaustive.check_quant(mod);exhaustive.reference();for(auto c:configs())exhaustive.validate(mod,c,true);
 }else{
  auto cs=configs();
  if(argc==7) {
   std::ifstream input(argv[6]);require(bool(input),"configuration file required");std::vector<Config> selected;std::string name;
   while(input>>name){auto it=std::find_if(cs.begin(),cs.end(),[&](Config c){return c.name()==name;});require(it!=cs.end(),"unknown frozen config");selected.push_back(*it);}
   require(!selected.empty(),"empty frozen set");cs=selected;
  }
  ...
  Data d(m,k,n,0);d.check_quant(mod);d.reference();for(auto c:cs)d.validate(mod,c,true);
  CUmodule old;CK(cuModuleLoad(&old,"archive/w4a16-r2-20260908/build/kernels.cubin"));
  CUmodule r3;CK(cuModuleLoad(&r3,"studies/w4a4-20260908/controls/w4a16-r3.cubin"));
  auto a16r3=[&]{
   CUdeviceptr native=0,dst=n==1 ? d.out.p:d.partial.p;
   void *args[]={&native,&d.dw.p,&d.ds.p,&d.din.p,&dst,&d.m,&d.g,&d.n};
   unsigned rr=m==17408 ? 2:1,nw=m==17408 ? 32:8;
   if(n==1)launch(fn(r3,m==17408 ? "r3_202_r2":"r3_200_r1"),(m+32*rr-1)/(32*rr),1,1,args,32*nw,32*rr*32*4);
   else {launch(fn(r3,"r3_112_r2"),(m+511)/512,(n+3)/4,32,args,256);void *red[]={&d.partial.p,&d.out.p,&d.m};launch(fn(mod,"reduce32"),(m+127)/128,n,1,red);}
  };
  ...
  a16r3();std::vector<float> current(baseline.size());CK(cuMemcpyDtoH(current.data(),d.out.p,current.size()*4));
  for(size_t i=0;i<current.size();i++)require(float_bits(current[i])==float_bits(baseline[i]),"r3 control mismatch");
  puts("CHECK current_a16_r3 PASS");
  ...
  do {for(auto c:cs)d.run(mod,c,true);a16();a16r3();CK(cuCtxSynchronize());}while(std::chrono::steady_clock::now()<until);
  for(int round=0;round<9;round++) {
   // Rotate both control and candidates through first/last positions.
   for(unsigned j=0;j<cs.size()+2;j++) {unsigned idx=(j+round)%(cs.size()+2);if(idx==cs.size()+1)printf("TIME round=%d config=current_a16_r3 us=%.6f\n",round,timed(a16r3,12));else if(idx==cs.size())printf("TIME round=%d config=sealed_a16 us=%.6f\n",round,timed(a16,12));
    else {auto c=cs[idx];printf("TIME round=%d config=%s us=%.6f\n",round,c.name().c_str(),timed([&]{d.run(mod,c,true);},12));}}
  }
  CK(cuModuleUnload(r3));CK(cuModuleUnload(old));
 }
