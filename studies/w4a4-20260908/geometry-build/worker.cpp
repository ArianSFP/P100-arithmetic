...
void launch(CUfunction f,unsigned x,unsigned y,unsigned z,void **args,unsigned threads=128){CK(cuLaunchKernel(f,x,y,z,threads,1,1,0,0,args,0));}
struct Config {int h,r,b,nw=4;std::string name()const{return "w4a4_h"+std::to_string(h)+"_r"+std::to_string(r)+"_b"+std::to_string(b)+(nw==4 ? "" : "_w"+std::to_string(nw));}};
...
struct Data {
 ...
 void run(CUmodule mod,Config c,bool prep) {
  ...
  launch(fn(mod,c.name().c_str()),(m+32*c.nw*c.r-1)/(32*c.nw*c.r),(n+c.b-1)/c.b,32,args,32*c.nw);
