...
int main(int argc,char**argv) {try {
 ...
 if(std::string(argv[2])=="check") {
  ...
  Data d(m,k,n,0);d.check_quant(mod);d.reference();for(auto c:configs())d.validate(mod,c,true);
  ...
  auto cs=configs();for(auto c:cs)for(int i=0;i<8;i++)d.run(mod,c,true);for(int i=0;i<8;i++){a16();a16r3();}CK(cuCtxSynchronize());
