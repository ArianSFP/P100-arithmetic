...
int main(int argc,char**argv)try{
 ...
 auto end=std::chrono::steady_clock::now()+std::chrono::seconds(ex==64?2:0);do{for(int b:{32,16,1024,2048})run(b,true);CU(cudaDeviceSynchronize());}while(std::chrono::steady_clock::now()<end);
 cudaEvent_t st,en;CU(cudaEventCreate(&st));CU(cudaEventCreate(&en));int reps=ex==64?7:1,iters=ex==64?6:1;int bs[]={32,16,1024,2048};
 for(int r=0;r<reps;++r)for(int i=0;i<4;++i){int b=bs[(i+r)%4];for(int prep=1;prep>=0;--prep){if(!prep&&b>=16)continue;CU(cudaEventRecord(st));for(int j=0;j<iters;++j)run(b,prep);CU(cudaEventRecord(en));CU(cudaEventSynchronize(en));float ms;CU(cudaEventElapsedTime(&ms,st,en));printf("TIME A%d prep=%d rep=%d us=%.9g\n",b,prep,r,ms*1000/iters);}}
 puts("PASS");return 0;
