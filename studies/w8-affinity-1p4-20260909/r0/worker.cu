...
int main(int argc,char**argv)try{
 ...
 auto run=[&](int b,bool prep){if(prep&&b<16)quant(b);if(b==64)w8_packed_half<true,1><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==128)w8_packed_half<true,4><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==256)w8_packed_half<true,0><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==32)aw_q8_service_m64_n128_halfpipe_sync<false><<<grid,256>>>(dd.p,dt.p,ts.size(),n,k);else if(b==16)aw_q8_service_m64_n128_halfpipe_sync<true><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==8)w8_lowact<8><<<grid,256>>>(dd8.p,dt.p,ts.size(),n,k);else w8_lowact<4><<<grid,256>>>(dd4.p,dt.p,ts.size(),n,k);CU(cudaGetLastError());};
 std::vector<float>ref;
 for(int b:{32,16,64,128,256}){CU(cudaMemset(out.p,0xff,oc*4));run(b,true);CU(cudaDeviceSynchronize());auto got=out.get();if(b==32)ref=got;auto&aa=b>=16?a:b==8?d8:d4;size_t nonfinite=0,changed=0,bad=0;double err=0,norm=0;for(size_t i=0;i<oc;++i){nonfinite+=!std::isfinite(got[i]);changed+=bits(got[i])!=bits(ref[i]);double d=double(got[i])-ref[i];err+=d*d;norm+=double(ref[i])*ref[i];}
  size_t samples=std::min<size_t>(oc,256);uint32_t sr=731;for(size_t z=0;z<samples;++z){size_t ix=rng(sr)%oc;int col=ix%n,row=(ix/n)%m,e=ix/(size_t(m)*n);float sum[2]={},total=0;int flush=b==64?1:b==128?4:0;for(int g=0;g<kg;++g){float d=__half2float(sc[(size_t(e)*n+col)*kg+g]);for(int j=0;j<32;++j){float weight=float(w[((size_t(e)*n+col)*kg+g)*32+j])*d;float activation=aa[(size_t(e)*m+row)*k+g*32+j];if(b>=64){weight=__half2float(__float2half_rn(weight));sum[j/16]=__half2float(__float2half_rn(std::fma(activation,weight,sum[j/16])));}else sum[j/16]=std::fma(activation,weight,sum[j/16]);}if(flush&&((g+1)%flush==0||g+1==kg)){total+=sum[0]+sum[1];sum[0]=sum[1]=0;}}float expected=flush?total:sum[0]+sum[1];bad+=bits(got[ix])!=bits(expected);}
 ...
 auto end=std::chrono::steady_clock::now()+std::chrono::seconds(ex==64?2:0);do{for(int b:{32,16,64,128,256})run(b,true);CU(cudaDeviceSynchronize());}while(std::chrono::steady_clock::now()<end);
 cudaEvent_t st,en;CU(cudaEventCreate(&st));CU(cudaEventCreate(&en));int reps=ex==64?7:1,iters=ex==64?6:1;int bs[]={32,16,64,128,256};
