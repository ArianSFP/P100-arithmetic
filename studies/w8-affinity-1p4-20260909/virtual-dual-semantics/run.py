...
ap=argparse.ArgumentParser();ap.add_argument('tag');ap.add_argument('--tokens',type=int,choices=[33,64,65,128,129,256,512],required=True);ap.add_argument('--projection',choices=['gu','down'],default='down');ap.add_argument('--memcheck',action='store_true');ap.add_argument('--ctas',type=int,choices=[1,2,3,4,5,6],default=2);ap.add_argument('--input-kind',type=int,choices=[0,1,2],default=0);ap.add_argument('--gather',type=int,choices=[0,1],default=0);a=ap.parse_args()
...
initial=health();tag='w8-affinity-semantics-'+a.tag
...
if a.memcheck:cmd+=['/usr/local/cuda-12.8/bin/compute-sanitizer','--tool','memcheck','--error-exitcode','99']
cmd += [str(p/'worker'),'--gpu-approved','1',str(a.tokens),'512' if a.projection=='gu' else '2048','2048' if a.projection=='gu' else '512','2' if a.memcheck else '64',str(a.ctas),str(a.input_kind),str(a.gather)]
