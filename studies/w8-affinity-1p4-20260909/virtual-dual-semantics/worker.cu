...
int main(int argc,char**argv)try{
 if(argc!=10||std::string(argv[1])!="--gpu-approved"||std::string(argv[2])!="1")throw std::runtime_error("usage: --gpu-approved 1 M N K experts");
 int input_kind=atoi(argv[8]),gather=atoi(argv[9]);if(input_kind<0||input_kind>2||gather<0||gather>1)throw std::runtime_error("input mode bounds");
 ...
 for(size_t i=0;i<ac;++i){if(input_kind==1){unsigned u=bits(a[i]);u+=0x7fff+((u>>16)&1);u&=0xffff0000;memcpy(&a[i],&u,4);}else if(input_kind==2)a[i]+=float((i%7)-3.0)*0.00013f;}
 ...
 std::vector<half>ha(ac);for(size_t i=0;i<ac;++i)ha[i]=input_kind==1?__ushort_as_half(bits(a[i])>>16):__float2half_rn(a[i]);Device<half>dh(ac);dh.put(ha);
 ...
 std::vector<int32_t>hrows(m);for(int i=0;i<m;++i)hrows[i]=m-1-i;Device<int32_t>dr(m);dr.put(hrows);
 std::vector<aw_work_desc>ds,ds16,ds8,ds4;std::vector<aw_tile_desc>ts;
 for(int e=0;e<ex;++e){aw_work_desc d={da.p+size_t(e)*m*k,(const char*)dw.p+size_t(e)*n*kg*34,out.p+size_t(e)*m*n,gather?dr.p:nullptr,0,m,0,e};ds.push_back(d);d.input=(const void*)((uintptr_t)(dh.p+size_t(e)*m*k)|(input_kind==1?0u:1u));ds16.push_back(d);d.input=aq8.p+size_t(e)*m*kg*36;ds8.push_back(d);d.input=aq4.p+size_t(e)*m*kg*20;ds4.push_back(d);for(int r=0;r<m;r+=64)ts.push_back({e,r,std::min(64,m-r)});}
 ...
 Device<int> dcount(1);dcount.put(std::vector<int>{int(ts.size())});
 auto run=[&](int b,bool prep){if(prep&&b<16)quant(b);if(b==64){if(input_kind==2){w8_fallback_half<false,1><<<prop.multiProcessorCount*3,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<false,1><<<candidate_grid,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);}else {w8_fallback_half<true,1><<<prop.multiProcessorCount*3,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<true,1><<<candidate_grid,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);}}else if(b==128){if(input_kind==2){w8_fallback_half<false,2><<<prop.multiProcessorCount*3,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<false,2><<<candidate_grid,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);}else {w8_fallback_half<true,2><<<prop.multiProcessorCount*3,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<true,2><<<candidate_grid,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);}}else if(b==256){if(input_kind==2){w8_fallback_half<false,4><<<prop.multiProcessorCount*3,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<false,4><<<candidate_grid,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);}else {w8_fallback_half<true,4><<<prop.multiProcessorCount*3,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<true,4><<<candidate_grid,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);}}else if(b==512){if(input_kind==2){w8_fallback_half<false,0><<<prop.multiProcessorCount*3,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<false,0><<<candidate_grid,256>>>(dd.p,dt.p,ts.size(),n,k,dcount.p);}else {w8_fallback_half<true,0><<<prop.multiProcessorCount*3,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);w8_pair_half<true,0><<<candidate_grid,256>>>(dd16.p,dt.p,ts.size(),n,k,dcount.p);}}else if(b==32)aw_q8_service_m64_n128_halfpipe_sync<false><<<grid,256>>>(dd.p,dt.p,ts.size(),n,k);else if(b==16)aw_q8_service_m64_n128_halfpipe_sync<true><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==8)w8_lowact<8><<<grid,256>>>(dd8.p,dt.p,ts.size(),n,k);else w8_lowact<4><<<grid,256>>>(dd4.p,dt.p,ts.size(),n,k);CU(cudaGetLastError());};
 std::vector<float>ref;
 for(int b:{32,16,64,128,256,512}){CU(cudaMemset(out.p,0xff,oc*4));run(b,true);CU(cudaDeviceSynchronize());auto got=out.get();if(b==32)ref=got;auto&aa=a;size_t nonfinite=0,changed=0,bad=0;double err=0,norm=0;for(size_t i=0;i<oc;++i){nonfinite+=!std::isfinite(got[i]);changed+=bits(got[i])!=bits(ref[i]);double d=double(got[i])-ref[i];err+=d*d;norm+=double(ref[i])*ref[i];}
  size_t samples=std::min<size_t>(oc,256);uint32_t sr=731;for(size_t z=0;z<samples;++z){size_t ix=rng(sr)%oc;int col=ix%n,row=(ix/n)%m,e=ix/(size_t(m)*n);if(gather)row=m-1-row;float sum[2]={},total=0;int flush=b==64?1:b==128?2:b==256?4:0;for(int g=0;g<kg;++g){float d=__half2float(sc[(size_t(e)*n+col)*kg+g]);for(int j=0;j<32;++j){float weight=float(w[((size_t(e)*n+col)*kg+g)*32+j])*d;float activation=aa[(size_t(e)*m+row)*k+g*32+j];if(b==16&&input_kind==2)activation=__half2float(__float2half_rn(activation));if(b>=64){activation=half_round(activation);weight=__half2float(__float2half_rn(weight));sum[j/16]=half_round(double(activation)*double(weight)+double(sum[j/16]));}else sum[j/16]=std::fma(activation,weight,sum[j/16]);}if(flush&&((g+1)%flush==0||g+1==kg)){total+=sum[0]+sum[1];sum[0]=sum[1]=0;}}float expected=flush?total:sum[0]+sum[1];bad+=bits(got[ix])!=bits(expected);}
  printf("CHECK A%d outputs=%zu samples=%zu bad=%zu nonfinite=%zu changed=%zu rel_l2=%.9g\n",b,oc,samples,bad,nonfinite,changed,std::sqrt(err/std::max(norm,1e-300)));fflush(stdout);if(bad||nonfinite||((b==32||(b==16&&input_kind!=2))&&changed))throw std::runtime_error("arithmetic mismatch");}
 printf("META M=%d N=%d K=%d experts=%d quantization_timed=1 full_q8_codes=1\n",m,n,k,ex);
 auto end=std::chrono::steady_clock::now()+std::chrono::seconds(ex==64?2:0);do{for(int b:{32,16,64,128,256,512})run(b,true);CU(cudaDeviceSynchronize());}while(std::chrono::steady_clock::now()<end);
 cudaEvent_t st,en;CU(cudaEventCreate(&st));CU(cudaEventCreate(&en));int reps=ex==64?7:1,iters=ex==64?6:1;int bs[]={32,16,64,128,256,512};
