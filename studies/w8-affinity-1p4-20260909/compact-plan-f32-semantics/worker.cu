...
int main(int argc,char**argv)try{
 ...
 for(size_t i=0;i<ac;++i)a[i]+=float((i%7)-3.0)*0.00013f;
 ...
 auto run=[&](int b,bool prep){if(prep&&b<16)quant(b);if(b>=64)w8_make_pairs<<<1,256>>>(dt.p,ts.size(),dtplan.p,dtleft.p,dcounts.p);if(b==64){w8_fallback_half<false,1><<<prop.multiProcessorCount*3,256>>>(dd.p,dtleft.p,ts.size(),n,k,dcounts.p+1);w8_pair_half<false,1><<<candidate_grid,256>>>(dd.p,dtplan.p,(ts.size()+1)/2,n,k,dcounts.p);}else if(b==128){w8_fallback_half<false,2><<<prop.multiProcessorCount*3,256>>>(dd.p,dtleft.p,ts.size(),n,k,dcounts.p+1);w8_pair_half<false,2><<<candidate_grid,256>>>(dd.p,dtplan.p,(ts.size()+1)/2,n,k,dcounts.p);}else if(b==256){w8_fallback_half<false,4><<<prop.multiProcessorCount*3,256>>>(dd.p,dtleft.p,ts.size(),n,k,dcounts.p+1);w8_pair_half<false,4><<<candidate_grid,256>>>(dd.p,dtplan.p,(ts.size()+1)/2,n,k,dcounts.p);}else if(b==512){w8_fallback_half<false,0><<<prop.multiProcessorCount*3,256>>>(dd.p,dtleft.p,ts.size(),n,k,dcounts.p+1);{if(n==512&&k==2048)w8_pair_half<false,0,512,2048><<<candidate_grid,256>>>(dd.p,dtplan.p,(ts.size()+1)/2,n,k,dcounts.p);else if(n==2048&&k==512)w8_pair_half<false,0,2048,512><<<candidate_grid,256>>>(dd.p,dtplan.p,(ts.size()+1)/2,n,k,dcounts.p);else w8_pair_half<false,0><<<candidate_grid,256>>>(dd.p,dtplan.p,(ts.size()+1)/2,n,k,dcounts.p);}}else if(b==32)aw_q8_service_m64_n128_halfpipe_sync<false><<<grid,256>>>(dd.p,dt.p,ts.size(),n,k);else if(b==16)aw_q8_service_m64_n128_halfpipe_sync<true><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==8)w8_lowact<8><<<grid,256>>>(dd8.p,dt.p,ts.size(),n,k);else w8_lowact<4><<<grid,256>>>(dd4.p,dt.p,ts.size(),n,k);CU(cudaGetLastError());};
