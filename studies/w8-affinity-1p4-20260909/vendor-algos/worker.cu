...
__global__ void decode_weights(const unsigned char* packed,half*out,size_t count,int n,int k){
...
int main(int argc,char**argv)try{
 if(argc!=7||std::string(argv[1])!="--gpu-approved"||std::string(argv[2])!="1")throw std::runtime_error("usage: --gpu-approved 1 M N K experts");
 ...
 Device<half>decoded(wc),hout(oc);cublasHandle_t handle;if(cublasCreate(&handle)!=CUBLAS_STATUS_SUCCESS)throw std::runtime_error("cublasCreate");
 ...
 auto run=[&](int b,bool prep){
  if(b==32)aw_q8_service_m64_n128_halfpipe_sync<false><<<grid,256>>>(dd.p,dt.p,ts.size(),n,k);
   ...
   cublasStatus_t st=cublasGemmStridedBatchedEx(handle,CUBLAS_OP_T,CUBLAS_OP_N,n,m,k,(b>=100&&b<200)?(const void*)&alpha16:(const void*)&alpha32,decoded.p,CUDA_R_16F,k,(long long)n*k,dh.p,CUDA_R_16F,k,(long long)m*k,(b>=100&&b<200)?(const void*)&beta16:(const void*)&beta32,(b>=100&&b<200)?(void*)hout.p:(void*)out.p,(b>=100&&b<200)?CUDA_R_16F:CUDA_R_32F,n,(long long)m*n,ex,(b>=100&&b<200)?CUBLAS_COMPUTE_16F:CUBLAS_COMPUTE_32F,(b>=100&&b<200)?(cublasGemmAlgo_t)(b-100):CUBLAS_GEMM_DEFAULT_TENSOR_OP);
   if(st==CUBLAS_STATUS_NOT_SUPPORTED||st==CUBLAS_STATUS_INVALID_VALUE){printf("SKIP A%d cublas_status=%d\n",b,int(st));return false;}
   if(st!=CUBLAS_STATUS_SUCCESS)throw std::runtime_error("cublasGemmStridedBatchedEx status="+std::to_string(int(st)));
   if((b>=100&&b<200))widen_output<<<4096,256>>>(hout.p,out.p,oc);
  }CU(cudaGetLastError());return true;};
 std::vector<int>variants={32,16,2048,199},valid;for(int algo=0;algo<24;++algo)variants.push_back(100+algo);
 std::vector<float>ref;
 for(int b:variants){CU(cudaMemset(out.p,0xff,oc*4));if(!run(b,true))continue;valid.push_back(b);CU(cudaDeviceSynchronize());auto got=out.get();if(b==32)ref=got;auto&aa=b>=16?a:b==8?d8:d4;size_t nonfinite=0,changed=0,bad=0;double err=0,norm=0;for(size_t i=0;i<oc;++i){nonfinite+=!std::isfinite(got[i]);changed+=bits(got[i])!=bits(ref[i]);double d=double(got[i])-ref[i];err+=d*d;norm+=double(ref[i])*ref[i];}
  size_t samples=std::min<size_t>(oc,256);uint32_t sr=731;for(size_t z=0;z<samples;++z){size_t ix=rng(sr)%oc;int col=ix%n,row=(ix/n)%m,e=ix/(size_t(m)*n);float sum[2]={};double exact=0,l1=0;for(int g=0;g<kg;++g){float d=__half2float(sc[(size_t(e)*n+col)*kg+g]);for(int j=0;j<32;++j){float weight=float(w[((size_t(e)*n+col)*kg+g)*32+j])*d;if(b>=100)weight=__half2float(__float2half_rn(weight));float activation=aa[(size_t(e)*m+row)*k+g*32+j];sum[j/16]=std::fma(activation,weight,sum[j/16]);exact+=double(activation)*weight;l1+=std::abs(double(activation)*weight);}}if(b>=100)bad+=std::abs(double(got[ix])-exact)>((b>=100&&b<200)?0.01:0.000002)*std::max(1.,l1);else bad+=bits(got[ix])!=bits(sum[0]+sum[1]);}
  printf("CHECK A%d outputs=%zu samples=%zu bad=%zu nonfinite=%zu changed=%zu rel_l2=%.9g\n",b,oc,samples,bad,nonfinite,changed,std::sqrt(err/std::max(norm,1e-300)));fflush(stdout);if(bad||nonfinite||((b==32||b==16)&&changed))throw std::runtime_error("arithmetic mismatch");}
 auto dwcheck=decoded.get();size_t decode_bad=0;for(size_t i=0;i<wc;++i){half expected=__float2half_rn(float(w[i])*__half2float(sc[i/32]));decode_bad+=__half_as_ushort(dwcheck[i])!=__half_as_ushort(expected);}printf("WEIGHT_DECODE values=%zu bad=%zu\n",wc,decode_bad);if(decode_bad)throw std::runtime_error("weight decoder mismatch");
 printf("META M=%d N=%d K=%d experts=%d quantization_timed=1 full_q8_codes=1\n",m,n,k,ex);
 auto end=std::chrono::steady_clock::now()+std::chrono::seconds(ex==64?2:0);do{for(int b:valid)run(b,true);CU(cudaDeviceSynchronize());}while(std::chrono::steady_clock::now()<end);
 cudaEvent_t st,en;CU(cudaEventCreate(&st));CU(cudaEventCreate(&en));int reps=ex==64?7:1,iters=ex==64?6:1;
 for(int r=0;r<reps;++r)for(int i=0;i<int(valid.size());++i){int b=valid[(i+r)%valid.size()];for(int prep=1;prep>=0;--prep){if(!prep&&b>=16)continue;CU(cudaEventRecord(st));for(int j=0;j<iters;++j)run(b,prep);CU(cudaEventRecord(en));CU(cudaEventSynchronize(en));float ms;CU(cudaEventElapsedTime(&ms,st,en));printf("TIME A%d prep=%d rep=%d us=%.9g\n",b,prep,r,ms*1000/iters);}}
 puts("PASS");return 0;
