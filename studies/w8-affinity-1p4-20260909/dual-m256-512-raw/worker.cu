...
int main(int argc,char**argv)try{
 ...
 std::vector<aw_tile_desc>ts256;for(int e=0;e<ex;++e)for(int r=0;r<m;r+=256)ts256.push_back({e,r,std::min(256,m-r)});Device<aw_tile_desc>dt256(ts256.size());dt256.put(ts256);
 auto run=[&](int b,bool prep){if(prep&&b<16)quant(b);if(b==64)w8_packed_half<true,1><<<candidate_grid,512>>>(dd16.p,dt256.p,ts256.size(),n,k);else if(b==128)w8_packed_half<true,2><<<candidate_grid,512>>>(dd16.p,dt256.p,ts256.size(),n,k);else if(b==256)w8_packed_half<true,4><<<candidate_grid,512>>>(dd16.p,dt256.p,ts256.size(),n,k);else if(b==512)w8_packed_half<true,0><<<candidate_grid,512>>>(dd16.p,dt256.p,ts256.size(),n,k);else if(b==32)aw_q8_service_m64_n128_halfpipe_sync<false><<<grid,256>>>(dd.p,dt.p,ts.size(),n,k);else if(b==16)aw_q8_service_m64_n128_halfpipe_sync<true><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==8)w8_lowact<8><<<grid,256>>>(dd8.p,dt.p,ts.size(),n,k);else w8_lowact<4><<<grid,256>>>(dd4.p,dt.p,ts.size(),n,k);CU(cudaGetLastError());};
