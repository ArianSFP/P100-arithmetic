...
int main(int argc,char**argv)try{
 ...
 for(int e=0;e<ex;++e){aw_work_desc d={da.p+size_t(e)*m*k,(const char*)dw.p+size_t(e)*n*kg*34,out.p+size_t(e)*m*n,nullptr,0,m,0,e};ds.push_back(d);d.input=(const void*)((uintptr_t)(dh.p+size_t(e)*m*k)|1u);ds16.push_back(d);d.input=aq8.p+size_t(e)*m*kg*36;ds8.push_back(d);d.input=aq4.p+size_t(e)*m*kg*20;ds4.push_back(d);for(int r=0;r<m;r+=32)ts.push_back({e,r,std::min(32,m-r)});}
 ...
 auto run=[&](int b,bool prep){if(b==32)aw_q8_service_m32<false,true,true><<<grid,128>>>(dd.p,dt.p,ts.size(),n,k);else if(b==16)aw_q8_service_m32<true,true,true><<<grid,128>>>(dd16.p,dt.p,ts.size(),n,k);else w8_m32_reduce<true,true,true><<<grid,128>>>(dd16.p,dt.p,ts.size(),n,k);CU(cudaGetLastError());};
 std::vector<float>ref;
 for(int b:{32,16,64}){CU(cudaMemset(out.p,0xff,oc*4));run(b,true);CU(cudaDeviceSynchronize());auto got=out.get();if(b==32)ref=got;auto&aa=b>=16?a:b==8?d8:d4;size_t nonfinite=0,changed=0,bad=0;double err=0,norm=0;for(size_t i=0;i<oc;++i){nonfinite+=!std::isfinite(got[i]);changed+=bits(got[i])!=bits(ref[i]);double d=double(got[i])-ref[i];err+=d*d;norm+=double(ref[i])*ref[i];}
  size_t samples=std::min<size_t>(oc,256);uint32_t sr=731;for(size_t z=0;z<samples;++z){size_t ix=rng(sr)%oc;int col=ix%n,row=(ix/n)%m,e=ix/(size_t(m)*n);float sum[4]={};for(int g=0;g<kg;++g){float d=__half2float(sc[(size_t(e)*n+col)*kg+g]);for(int j=0;j<32;++j)sum[j/8]=std::fma(aa[(size_t(e)*m+row)*k+g*32+j],float(w[((size_t(e)*n+col)*kg+g)*32+j])*d,sum[j/8]);}bad+=bits(got[ix])!=bits(((sum[0]+sum[1])+sum[2])+sum[3]);}
  printf("CHECK A%d outputs=%zu samples=%zu bad=%zu nonfinite=%zu changed=%zu rel_l2=%.9g\n",b,oc,samples,bad,nonfinite,changed,std::sqrt(err/std::max(norm,1e-300)));fflush(stdout);if(bad||nonfinite||changed)throw std::runtime_error("arithmetic mismatch");}
 printf("META M=%d N=%d K=%d experts=%d quantization_timed=1 full_q8_codes=1\n",m,n,k,ex);
 auto end=std::chrono::steady_clock::now()+std::chrono::seconds(ex==64?2:0);do{for(int b:{32,16,64})run(b,true);CU(cudaDeviceSynchronize());}while(std::chrono::steady_clock::now()<end);
 cudaEvent_t st,en;CU(cudaEventCreate(&st));CU(cudaEventCreate(&en));int reps=ex==64?7:1,iters=ex==64?6:1;int bs[]={32,16,64};
 for(int r=0;r<reps;++r)for(int i=0;i<3;++i){int b=bs[(i+r)%3];for(int prep=1;prep>=0;--prep){if(!prep&&b>=16)continue;CU(cudaEventRecord(st));for(int j=0;j<iters;++j)run(b,prep);CU(cudaEventRecord(en));CU(cudaEventSynchronize(en));float ms;CU(cudaEventElapsedTime(&ms,st,en));printf("TIME A%d prep=%d rep=%d us=%.9g\n",b,prep,r,ms*1000/iters);}}
 puts("PASS");return 0;
