...
ap=argparse.ArgumentParser();ap.add_argument('tag');ap.add_argument('--tokens',type=int,choices=[16,31,32,33,64,128,256,512],required=True);ap.add_argument('--projection',choices=['gu','down'],default='down');ap.add_argument('--memcheck',action='store_true');a=ap.parse_args()
...
initial=health();tag='w8-m32-reduce-'+a.tag
note(f'HELD: {tag}; GPU{device} device lock only per user instruction, W8A16 exact M32 reduction batching against unchanged M32; bounded watchdog worker.')
