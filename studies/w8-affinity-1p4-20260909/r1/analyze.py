...
for proj in ['gu','down']:
 for r in range(3):
  ...
  assert len(checks)==5 and all(int(c[3])==int(c[4])==0 for c in checks)
  samples={}
  for b,prep,rep,us in re.findall(r'TIME A(\d+) prep=(\d+) rep=(\d+) us=([\d.]+)',raw):
   if int(rep)>0:samples.setdefault(b,[]).append(float(us))
  assert len(samples)==5 and all(len(x)==6 for x in samples.values())
...
for proj in ['gu','down']:
 ...
 summary[proj]={'base_us':st.median(r['base_us'] for r in rr),'candidates':{b:{'us':st.median(r['us'][b] for r in rr),'speedup':st.median(r['speedup'][b] for r in rr),'rel_l2':rr[0]['rel_l2'][b]} for b in ['64','128','256']}}
