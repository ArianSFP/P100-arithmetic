...
ap=argparse.ArgumentParser();ap.add_argument('tag');ap.add_argument('--tokens',type=int,choices=[33,64,128,256,512],required=True);ap.add_argument('--projection',choices=['gu','down'],default='down');ap.add_argument('--memcheck',action='store_true');ap.add_argument('--ctas',type=int,choices=[1,2,3,4,5,6],default=2);a=ap.parse_args()
...
initial=health();tag='w8-affinity-m128-half-b-bound2-'+a.tag
...
cmd += [str(p/'worker'),'--gpu-approved','1',str(a.tokens),'512' if a.projection=='gu' else '2048','2048' if a.projection=='gu' else '512','2' if a.memcheck else '64',str(a.ctas)]
