...
int main(int argc,char**argv)try{
 ...
 auto end=std::chrono::steady_clock::now()+std::chrono::seconds(ex==64&&k>=512?2:0);do{for(int b:{32,16,512})run(b,true);CU(cudaDeviceSynchronize());}while(std::chrono::steady_clock::now()<end);
 cudaEvent_t st,en;CU(cudaEventCreate(&st));CU(cudaEventCreate(&en));int reps=ex==64&&k>=512?7:1,iters=ex==64&&k>=512?6:1;int bs[]={32,16,512};
