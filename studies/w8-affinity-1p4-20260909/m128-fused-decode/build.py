...
(p/'manifest.json').write_text(json.dumps({'command':cmd,'hashes':{f:hashlib.sha256((p/f).read_bytes()).hexdigest() for f in ['worker','worker.cu','q8.inc','lowact.inc','packed.inc','decode.inc','build.py']}},indent=2)+'\n')
