...
int main(int argc,char**argv)try{
 ...
 unsigned *decode_bad;CU(cudaMalloc(&decode_bad,4));CU(cudaMemset(decode_bad,0,4));w8_check_decode<<<4096,256>>>(decode_bad);unsigned db;CU(cudaMemcpy(&db,decode_bad,4,cudaMemcpyDeviceToHost));CU(cudaFree(decode_bad));printf("DECODE_EXHAUSTIVE cases=16252928 bad=%u\n",db);if(db)throw std::runtime_error("fused decoder mismatch");
