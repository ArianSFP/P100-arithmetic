...
#include "packed.inc"
#include "packed-single.inc"
#include "virtual-packed.inc"
...
static std::atomic<int> w8_validation_mode{[](){const char*v=getenv("GGML_CUDA_AW_W8_MODE");int mode=v?atoi(v):0;GGML_ASSERT(mode>=0&&mode<=6);return mode;}()};
static std::atomic<unsigned long long> w8_validation_calls[7];
template<bool BF16_INPUT>
static void aw_dispatch_w8(int blocks,cudaStream_t stream,const aw_work_desc *d,const aw_tile_desc *t,int count,int n,int k,const int *count_dev=nullptr){
 ...
 else if(mode==2)w8_single_half<BF16_INPUT,2><<<blocks,256,0,stream>>>(d,t,count,n,k,count_dev);
 else if(mode==3)w8_single_half<BF16_INPUT,4><<<blocks,256,0,stream>>>(d,t,count,n,k,count_dev);
 else if(mode==4)w8_packed_half<BF16_INPUT,0><<<blocks,256,0,stream>>>(d,t,count,n,k,count_dev);
 else if(mode==6)aw_dispatch_w8_planned<BF16_INPUT>(blocks,stream,d,t,count,n,k,count_dev);
 else {w8_fallback_half<BF16_INPUT,0><<<blocks*3/2,256,0,stream>>>(d,t,count,n,k,count_dev);w8_pair_half<BF16_INPUT,0><<<blocks,256,0,stream>>>(d,t,count,n,k,count_dev);}
}
...
static void aw_live_launch_projection(
    ...
    if (aw_q8_cohortrail_enabled()) {
        ...
        if (bf16_input) {
            aw_dispatch_w8<true>(persistent_blocks,stream,
                            desc_ptr,
                            (const aw_tile_desc *)
                                state.tiles_m64.get(),
                            int(state.tiles_m64.size()/sizeof(aw_tile_desc)), n, k, tile_count + 0);
            ...
            aw_dispatch_w8<false>(persistent_blocks,stream,
                            desc_ptr,
                            (const aw_tile_desc *)
                                state.tiles_m64.get(),
                            int(state.tiles_m64.size()/sizeof(aw_tile_desc)), n, k, tile_count + 0);
...
extern "C" __attribute__((visibility("default"))) void aw_w8_set_mode(int mode){GGML_ASSERT(mode>=0&&mode<=6);w8_validation_mode.store(mode);}
extern "C" __attribute__((visibility("default"))) unsigned long long aw_w8_calls(int mode){GGML_ASSERT(mode>=0&&mode<=6);return w8_validation_calls[mode].load();}
