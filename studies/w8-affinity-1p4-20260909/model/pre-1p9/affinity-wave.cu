...
static std::atomic<unsigned long long> w8_validation_calls[6];
template<bool BF16_INPUT>
...
extern "C" __attribute__((visibility("default"))) void aw_w8_set_mode(int mode){GGML_ASSERT(mode>=0&&mode<=5);w8_validation_mode.store(mode);}
extern "C" __attribute__((visibility("default"))) unsigned long long aw_w8_calls(int mode){GGML_ASSERT(mode>=0&&mode<=5);return w8_validation_calls[mode].load();}
