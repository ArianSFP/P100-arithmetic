...
parser=argparse.ArgumentParser();parser.add_argument('tag');parser.add_argument('--kind',choices=['semantic','quality','prefill','decode','server'],required=True);parser.add_argument('--control',action='store_true');parser.add_argument('--native',action='store_true');parser.add_argument('--partial',choices=['f32','bf16']);parser.add_argument('--diagonal',choices=['legacy','panel2048']);parser.add_argument('--plan-cache',choices=['0','1']);parser.add_argument('--memcheck',action='store_true');parser.add_argument('--tokens',type=int,choices=[2048,8128,8192],default=8192);parser.add_argument('--f32weights',action='store_true');parser.add_argument("--w8-mode",type=int,choices=range(7),default=0);args=parser.parse_args()
...
env['GGML_CUDA_AW_W8_MODE']=str(args.w8_mode)
