...
for m in [64,128,256,512]:
 for proj in ['gu','down']:
  workers=[]
  for r in range(3):
   base=p/'r1' if m==256 else p;tag=f'{proj}-r{r}' if m==256 else f'm{m}-{proj}-r{r}'
   ...
   ck=re.findall(r'CHECK A(\d+) outputs=(\d+) samples=(\d+) bad=(\d+) nonfinite=(\d+) changed=(\d+) rel_l2=([\d.e+-]+)',raw);assert len(ck)==5 and all(int(c[3])==int(c[4])==0 for c in ck)
   d={}
   ...
   workers.append({'tag':tag,'base_us':ref,'us':med,'speedup':{b:ref/med[b] for b in ['64','128','256']},'rel_l2':{c[0]:float(c[-1]) for c in ck}})
  row={'m':m,'projection':proj,'base_us':st.median(w['base_us'] for w in workers),'speedup':{b:st.median(w['speedup'][b] for w in workers) for b in ['64','128','256']},'workers':workers};rows.append(row)
