...
int main(int argc,char**argv)try{
 ...
 std::vector<half>wh(wc);for(int e=0;e<ex;++e)for(int r=0;r<n;++r)for(int g=0;g<kg;++g)for(int j=0;j<32;++j){size_t si=(size_t(e)*n+r)*kg+g;size_t dst=((size_t(e)*(n/64)+r/64)*kg+g)*2048+(r%64)*32+j;wh[dst]=__float2half_rn(float(w[si*32+j])*__half2float(sc[si]));}Device<half>dwh(wc);dwh.put(wh);
 ...
 std::vector<aw_work_desc>ds,ds16,ds8,ds4,dsw;std::vector<aw_tile_desc>ts;
 for(int e=0;e<ex;++e){aw_work_desc d={da.p+size_t(e)*m*k,(const char*)dw.p+size_t(e)*n*kg*34,out.p+size_t(e)*m*n,nullptr,0,m,0,e};ds.push_back(d);d.input=(const void*)((uintptr_t)(dh.p+size_t(e)*m*k)|1u);ds16.push_back(d);auto wd=d;wd.weight=(const char*)(dwh.p+size_t(e)*n*k);dsw.push_back(wd);d.input=aq8.p+size_t(e)*m*kg*36;ds8.push_back(d);d.input=aq4.p+size_t(e)*m*kg*20;ds4.push_back(d);for(int r=0;r<m;r+=64)ts.push_back({e,r,std::min(64,m-r)});}
 Device<aw_work_desc>ddw(dsw.size()),dd(ds.size()),dd16(ds16.size()),dd8(ds8.size()),dd4(ds4.size());Device<aw_tile_desc>dt(ts.size());ddw.put(dsw);dd.put(ds);dd16.put(ds16);dd8.put(ds8);dd4.put(ds4);dt.put(ts);
 auto run=[&](int b,bool prep){if(prep&&b<16)quant(b);if(b==64)w16_reference<true><<<grid,256>>>(ddw.p,dt.p,ts.size(),n,k);else if(b==32)aw_q8_service_m64_n128_halfpipe_sync<false><<<grid,256>>>(dd.p,dt.p,ts.size(),n,k);else if(b==16)aw_q8_service_m64_n128_halfpipe_sync<true><<<grid,256>>>(dd16.p,dt.p,ts.size(),n,k);else if(b==8)w8_lowact<8><<<grid,256>>>(dd8.p,dt.p,ts.size(),n,k);else w8_lowact<4><<<grid,256>>>(dd4.p,dt.p,ts.size(),n,k);CU(cudaGetLastError());};
 std::vector<float>ref;
 for(int b:{32,16,8,4,64}){CU(cudaMemset(out.p,0xff,oc*4));run(b,true);CU(cudaDeviceSynchronize());auto got=out.get();if(b==32)ref=got;auto&aa=b>=16?a:b==8?d8:d4;size_t nonfinite=0,changed=0,bad=0;double err=0,norm=0;for(size_t i=0;i<oc;++i){nonfinite+=!std::isfinite(got[i]);changed+=bits(got[i])!=bits(ref[i]);double d=double(got[i])-ref[i];err+=d*d;norm+=double(ref[i])*ref[i];}
  size_t samples=std::min<size_t>(oc,256);uint32_t sr=731;for(size_t z=0;z<samples;++z){size_t ix=rng(sr)%oc;int col=ix%n,row=(ix/n)%m,e=ix/(size_t(m)*n);float sum[2]={};for(int g=0;g<kg;++g){float d=__half2float(sc[(size_t(e)*n+col)*kg+g]);for(int j=0;j<32;++j)sum[j/16]=std::fma(aa[(size_t(e)*m+row)*k+g*32+j],(b==64?__half2float(__float2half_rn(float(w[((size_t(e)*n+col)*kg+g)*32+j])*d)):float(w[((size_t(e)*n+col)*kg+g)*32+j])*d),sum[j/16]);}bad+=bits(got[ix])!=bits(sum[0]+sum[1]);}
 ...
 auto end=std::chrono::steady_clock::now()+std::chrono::seconds(ex==64?2:0);do{for(int b:{32,16,8,4,64})run(b,true);CU(cudaDeviceSynchronize());}while(std::chrono::steady_clock::now()<end);
 cudaEvent_t st,en;CU(cudaEventCreate(&st));CU(cudaEventCreate(&en));int reps=ex==64?7:1,iters=ex==64?6:1;int bs[]={32,16,8,4,64};
 for(int r=0;r<reps;++r)for(int i=0;i<5;++i){int b=bs[(i+r)%5];for(int prep=1;prep>=0;--prep){if(!prep&&b>=16)continue;CU(cudaEventRecord(st));for(int j=0;j<iters;++j)run(b,prep);CU(cudaEventRecord(en));CU(cudaEventSynchronize(en));float ms;CU(cudaEventElapsedTime(&ms,st,en));printf("TIME A%d prep=%d rep=%d us=%.9g\n",b,prep,r,ms*1000/iters);}}
 puts("PASS");return 0;
