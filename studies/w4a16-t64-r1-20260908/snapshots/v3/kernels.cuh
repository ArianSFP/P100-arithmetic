...
template<int R, int NW, int NT, int MODE=0>
...
void q4_token_pairs(const half *__restrict__ a, const unsigned char *__restrict__ w,
    ...
    __shared__ half2 sb[(MODE&1)?1:2][32][NT];
    ...
    for(int work=blockIdx.x;work<experts*tm*ng;work+=gridDim.x) {
        ...
        auto fetch=[&](int g, uint4 &av, unsigned char (&bv)[BLOAD], half &d) {
            ...
            if constexpr(MODE&4) {
                const unsigned char *src=w+b+128+(br%64)*16+bk;
                #pragma unroll
                for(int i=0;i<BLOAD;i+=4) *reinterpret_cast<uint32_t *>(&bv[i])=*reinterpret_cast<const uint32_t *>(src+i);
            } else {
                #pragma unroll
        ...
        auto stage=[&](int buf, const uint4 &av, const unsigned char (&bv)[BLOAD], half d) {
            ...
            for(int i=0;i<BLOAD;++i) {
                sb[(MODE&1)?0:buf][bk+i][br]=__half2half2(__int2half_rn(int(bv[i]&15)-8));
                sb[(MODE&1)?0:buf][bk+i+16][br]=__half2half2(__int2half_rn(int(bv[i]>>4)-8));
            }
            if(bk==0) sd[buf][br]=d;
        };
        ...
        for(int g=0;g<groups;++g) {
            ...
            if constexpr(R==0) {
                ...
                for(int kk=0;kk<32;++kk) {
                    ...
                    #pragma unroll
                    for(int j=0;j<J;++j) {
                        const float q=__half2float(__low2half(sb[(MODE&1)?0:buf][kk][cn+j]));
                        #pragma unroll
                        for(int i=0;i<2;++i) {
                ...
                #pragma unroll 1
                for(int chunk=0;chunk<((MODE&2)?4:1);++chunk) {
                #pragma unroll
                for(int step=0;step<((MODE&2)?8:32);++step) {
                    const int kk=chunk*8+step;
                    const half2 x[2]={*reinterpret_cast<const half2 *>(&sa[buf][kk][2*rm]),*reinterpret_cast<const half2 *>(&sa[buf][kk][2*rm+2])};
                    half2 bvec[J];
                    #pragma unroll
                    for(int j=0;j<J;j+=4) *reinterpret_cast<uint4 *>(&bvec[j])=*reinterpret_cast<const uint4 *>(&sb[(MODE&1)?0:buf][kk][cn+j]);
                    #pragma unroll
                    for(int j=0;j<J;++j) {
                        const half2 q=bvec[j];
                        #pragma unroll
                        for(int i=0;i<2;++i) h[step%R][i][j]=__hfma2(q,x[i],h[step%R][i][j]);
                    }
                }
                }
                #pragma unroll
