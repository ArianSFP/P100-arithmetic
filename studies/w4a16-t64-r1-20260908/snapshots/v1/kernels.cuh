...
void q4_token_pairs(const half *__restrict__ a, const unsigned char *__restrict__ w,
    ...
    __shared__ half2 sa[2][32][MT/2];
    __shared__ half sb[2][32][NT];
    __shared__ half sd[2][NT];
    const int tid=threadIdx.x, warp=tid/32, lane=tid%32;
    const int pair=tid%(MT/2), ak=4*(tid/(MT/2));
    ...
    for(int work=blockIdx.x;work<experts*tm*ng;work+=gridDim.x) {
        ...
        const int r0=mt*MT+2*pair;
        const size_t abase=size_t(e)*tokens*k;
        const size_t wbase=size_t(e)*(n/64)*groups*1152;
        float2 acc[2][J]={};
        auto fetch=[&](int g, half2 (&av)[4], unsigned char (&bv)[BLOAD], half &d) {
            #pragma unroll
            for(int i=0;i<4;++i) {
                half lo=r0<tokens?a[abase+size_t(r0)*k+g*32+ak+i]:__ushort_as_half(0);
                half hi=r0+1<tokens?a[abase+size_t(r0+1)*k+g*32+ak+i]:__ushort_as_half(0);
                av[i]=__halves2half2(lo,hi);
            }
        ...
        auto stage=[&](int buf, const half2 (&av)[4], const unsigned char (&bv)[BLOAD], half d) {
            #pragma unroll
            for(int i=0;i<4;++i) sa[buf][ak+i][pair]=av[i];
            #pragma unroll
            for(int i=0;i<BLOAD;++i) {
                sb[buf][bk+i][br]=__int2half_rn(int(bv[i]&15)-8);
                sb[buf][bk+i+16][br]=__int2half_rn(int(bv[i]>>4)-8);
            }
            if(bk==0) sd[buf][br]=d;
        };
        half2 av[4]; unsigned char bv[BLOAD]; half d;
        ...
        for(int g=0;g<groups;++g) {
            ...
            if constexpr(R==0) {
                ...
                for(int kk=0;kk<32;++kk) {
                    const float2 x[2]={__half22float2(sa[buf][kk][rm]),__half22float2(sa[buf][kk][rm+1])};
                    #pragma unroll
                    for(int j=0;j<J;++j) {
                        const float q=__half2float(sb[buf][kk][cn+j]);
                        #pragma unroll
                        for(int i=0;i<2;++i) {
                ...
                #pragma unroll
                for(int kk=0;kk<32;++kk) {
                    const half2 x[2]={sa[buf][kk][rm],sa[buf][kk][rm+1]};
                    #pragma unroll
                    for(int j=0;j<J;++j) {
                        const half2 q=__half2half2(sb[buf][kk][cn+j]);
                        #pragma unroll
