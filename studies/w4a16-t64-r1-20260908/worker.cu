...
struct Config {int r,nw,nt;std::string name;int mode=0;};
...
void launch(const Config&c,const half*a,const unsigned char*w,float*out,int t,int n,int k,int e,int grid,
    ...
    #define RUNM(R,NW,NT,M) if(c.r==R&&c.nw==NW&&c.nt==NT&&c.mode==M){q4_token_pairs<R,NW,NT,M><<<((M&8)?grid*3/2:grid),NW*32>>>(a,w,out,t,n,k,e);return;}
    #define RUN(R,NW,NT) RUNM(R,NW,NT,0)
    #define TILE(NW,NT) RUN(0,NW,NT) RUN(1,NW,NT) RUN(2,NW,NT) RUN(4,NW,NT) RUN(8,NW,NT)
    TILE(4,64) TILE(8,64) TILE(8,128)
    #define OPT(NW,NT) RUNM(2,NW,NT,3) RUNM(2,NW,NT,7) RUNM(2,NW,NT,15) RUNM(1,NW,NT,7) RUNM(4,NW,NT,7)
    OPT(4,64) OPT(8,64) OPT(4,128) OPT(8,128)
    #undef OPT
    #undef RUNM
    #undef TILE
    #undef RUN
...
int main(int argc,char**argv) try {
    int t=64,n=512,k=2048,experts=8,reps=9,iters=8,seed=3911,pattern=0,gridmul=2;
    std::string only;
    for(int i=1;i<argc;i+=2){if(i+1==argc)throw std::runtime_error("missing argument");std::string key=argv[i];
        ...
        else if(key=="--seed")seed=v;else if(key=="--pattern")pattern=v;else if(key=="--grid")gridmul=v;else throw std::runtime_error("unknown argument");
    }
    if(t<1||t>512||n<128||n>2048||n%128||k<32||k>2048||k%32||experts<1||experts>64||reps<1||reps>30||iters<1||iters>30||pattern<0||pattern>3||gridmul<1||gridmul>8)throw std::runtime_error("outside reviewed shape limits");
    ...
    std::vector<unsigned char>w4(wc/32*18,0),w4row(wc/32*18,0),w8(wc/32*34,0);
    uint32_t rs=seed;
    ...
    for(int e=0;e<experts;++e)for(int row=0;row<n;++row)for(int g=0;g<groups;++g){
        ...
        memcpy(w4.data()+b4+2*(row%64),&scale,2);memcpy(w4row.data()+b4+2*(row%64),&scale,2);memcpy(w8.data()+b8+2*(row%64),&scale,2);
        for(int j=0;j<32;++j){int q=pattern==3?-8:int(rng(rs)%16)-8;codes[bg*32+j]=int8_t(q);
            w4[b4+128+(j%16)*64+row%64]|=uint8_t(q+8)<<(j<16?0:4);
            w4row[b4+128+(row%64)*16+j%16]|=uint8_t(q+8)<<(j<16?0:4);
            w8[b8+128+(row%64)*32+j]=uint8_t(int8_t(q));}
    }
    Device<float> da(ac),out(oc);Device<half> dh(ac);Device<unsigned char>dw4(w4.size()),dw4row(w4row.size()),dw8(w8.size());
    da.put(a);dh.put(ha);dw4.put(w4);dw4row.put(w4row);dw8.put(w8);
    ...
    for(auto shape:std::vector<std::pair<int,int>>{{4,64},{8,64},{4,128},{8,128}})for(auto rm:std::vector<std::pair<int,int>>{{2,3},{2,7},{2,15},{1,7},{4,7}})
        cfg.push_back({rm.first,shape.first,shape.second,"q4_r"+std::to_string(rm.first)+"_w"+std::to_string(shape.first)+"_n"+std::to_string(shape.second)+"_m"+std::to_string(rm.second),rm.second});
    ...
    auto run=[&](const Config&c,bool prep){if(prep&&c.r>=0)prepare_a16<<<std::min<size_t>((ac+255)/256,4096),256>>>(da.p,dh.p,ac);
        launch(c,dh.p,(c.mode&4)?dw4row.p:dw4.p,out.p,t,n,k,experts,c.r<0?grid:prop.multiProcessorCount*gridmul,dd.p,dt.p,int(tiles.size()));CU(cudaGetLastError());};
    printf("META tokens=%d n=%d k=%d experts=%d seed=%d pattern=%d sm=%d candidate_grid=%d\n",t,n,k,experts,seed,pattern,prop.multiProcessorCount,gridmul);fflush(stdout);
