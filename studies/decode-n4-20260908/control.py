...
TOKEN='decode-n4-'+str(time.time_ns())+'-gpu1'
...
lock=open('/tmp/P100-arithmetic-gpu1.lock','a');deadline=time.monotonic()+900
while True:
 try:fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB);break
 except BlockingIOError:
  if time.monotonic()>deadline:raise RuntimeError('Lock wait expired; no CUDA execution')
  time.sleep(1)
guard();h=health();append('HELD: '+TOKEN+'; GPU1-only four-token decode split-K study, GPU1-specific lock held; user explicitly authorizes use of unreserved devices alongside GPU3 work. GPU0/2/3 untouched. No production edits.')
