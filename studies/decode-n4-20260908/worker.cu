...
struct Cfg {std::string name;int mode,r,b,nw,s;const void* kernel;bool fused;int lanes=1;int partitions=1;bool raw=false;bool packed=false;};
std::vector<Cfg> cfgs;
...
template<int R,int S,int P,int W>void addwide(){
#define ADDU(M) cfgs.push_back({"u"+std::to_string(M)+"r"+std::to_string(R)+"s"+std::to_string(S)+"p"+std::to_string(P)+"w"+std::to_string(W),M,R,4,W,S,(void*)decode_parts<M,R,4,W,S,P>,true,1,P})
ADDU(3);ADDU(4);ADDU(5);ADDU(9);
#undef ADDU
}
template<int R,int S,int P>void addraw(){constexpr int W=S/P<8?S/P:8;
#define ADDRAW(M) cfgs.push_back({"f"+std::to_string(M)+"r"+std::to_string(R)+"s"+std::to_string(S)+"p"+std::to_string(P),M,R,4,W,S,(void*)decode_parts<M,R,4,W,S,P>,true,1,P,true})
ADDRAW(16);ADDRAW(17);ADDRAW(21);
#undef ADDRAW
}
template<int R,int S,int P>void addpacked(){constexpr int W=S/P<8?S/P:8;
#define ADDPACK(M) cfgs.push_back({"v"+std::to_string(M)+"r"+std::to_string(R)+"s"+std::to_string(S)+"p"+std::to_string(P),M,R,4,W,S,(void*)decode_parts<M,R,4,W,S,P>,true,1,P,false,true})
ADDPACK(28);ADDPACK(29);
#undef ADDPACK
}
...
#undef ALLOC
 ...
 void launch(const Cfg&c){require(c.nw*32<=1024&&(!c.fused||c.s/c.partitions*c.r*c.b*32*4<=49152),"launch bounds");if(c.packed){require(prep,"paired layout requires charged preparation");prepare_paired<<<(n*k/2+127)/128,128>>>(draw,da,n*k/2);}else if(prep&&!c.raw)prepare<<<(n*k+255)/256,256>>>(draw,da,n*k);float*dst=c.fused&&c.partitions==1?out:partial;uint16_t*act=c.raw?reinterpret_cast<uint16_t*>(draw):da;void*args[]={&dq,&dw,&ds,&act,&dst,&m,&g,&n};unsigned tile=c.r*32/c.lanes*(c.fused?1:c.nw);CK(cudaLaunchKernel(c.kernel,dim3((m+tile-1)/tile,(n+c.b-1)/c.b,c.fused?c.partitions:c.s),dim3(c.nw*32),args,c.fused?c.s/c.partitions*c.r*c.b*32*4:0));if(!c.fused)reduce32<<<dim3((m+127)/128,n),128>>>(partial,out,m);if(c.partitions==2)reduce_parts<2><<<dim3((m+127)/128,n),128>>>(partial,out,m);
 ...
 float oracle(const Cfg&c,unsigned row,unsigned b){float parts[4]={};for(int part=0;part<c.lanes;part++){float acc[32]={};for(int stripe=0;stripe<c.s;stripe++){float h0=0,h1=0;for(unsigned z=stripe;z<g;z+=c.s){auto&v=q[size_t(row)*g+z];float dot=0,x0=0,x1=0;for(int jj=part*(32/c.lanes);jj<(part+1)*(32/c.lanes);jj++){int j=c.mode>=3&&c.mode%12!=9 ? (jj/8)*8+(jj%8)/2+(jj%2)*4 : jj;float x=half_float(a[b*k+z*32+j]),weight=native_code(v,j);if(c.mode<0||c.mode%3==0)dot=std::fma(weight,x,dot);else if(jj&1)x1=hf(double(weight)*x+x1);else x0=hf(double(weight)*x+x0);}float d=half_float(v.d);if(c.mode%3==2){h0=hf(double(x0)*d+h0);h1=hf(double(x1)*d+h1);}else{if(c.mode%3==1)dot=x0+x1;acc[stripe]=std::fma(dot,d,acc[stripe]);}}if(c.mode%3==2)acc[stripe]=h0+h1;}parts[part]=tree_sum(acc,c.s);}return tree_sum(parts,c.lanes);}
};
int main(int argc,char**argv){
 ...
 partset<4,8>();partset<4,16>();partset<4,32>();
 addwide<2,32,2,16>();addwide<2,16,2,4>();addwide<2,32,4,4>();addwide<4,32,2,16>();addwide<4,32,4,4>();
 add<2,4,16,16>();add<2,4,16,32>();add<2,4,32,32>();
 addparts<8,16,2>();addparts<8,16,4>();addparts<8,16,8>();addparts<8,32,4>();addparts<8,32,8>();
 addraw<2,32,4>();addraw<4,32,4>();addraw<4,32,8>();addraw<8,32,4>();addraw<8,32,8>();
 addglobal<1,4,4>();
 addparts<3,32,4>();addparts<3,32,8>();addparts<6,32,4>();addparts<6,32,8>();
 addraw<3,32,4>();addraw<3,32,8>();addraw<6,32,4>();addraw<6,32,8>();
 addpacked<2,32,4>();addpacked<3,32,4>();addpacked<3,32,8>();addpacked<4,32,4>();addpacked<4,32,8>();addpacked<6,32,8>();
 ...
 for(auto&c:cfgs){auto v=d.get(c);original.push_back(v);
  ...
  if((c.mode<=0||c.mode%12==9)&&c.s==32&&changed){fprintf(stderr,"Exact baseline failure\n");return 6;}
