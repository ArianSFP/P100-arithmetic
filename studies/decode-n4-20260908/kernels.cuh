...
__global__ void decode_parts(const Q4Block *native,const uint32_t *__restrict__ words,const uint16_t *__restrict__ scales,const uint16_t *__restrict__ a,float *__restrict__ out,unsigned rows,unsigned groups,unsigned batches) {
    ...
    for(unsigned stripe=warp;stripe<S/P;stripe+=NW){
        ...
        for(unsigned g=stripe*P+blockIdx.z;g<groups;g+=S){
            ...
            #pragma unroll
            for(int b=0;b<B;b++){
                if constexpr(Mode%12==9)lane_a[b]=batch0+b<batches ? (Mode>=12 ? __half2float(__float2half_rn(reinterpret_cast<const float*>(a)[((batch0+b)*groups+g)*32+lane])) : widen(a[((batch0+b)*groups+g)*32+lane])):0;
                else if constexpr(Mode<3)ap[b]=batch0+b<batches ? reinterpret_cast<const unsigned*>(a)[((batch0+b)*groups+g)*16+(lane&15)] : 0;
                else{unsigned i=((batch0+b)*groups+g)*32+(lane&15)/4*8+(lane&3);if constexpr(Mode>=24)ap[b]=batch0+b<batches ? reinterpret_cast<const unsigned*>(a)[((batch0+b)*groups+g)*16+(lane&15)]:0;
                else if constexpr(Mode>=12){const float*raw=reinterpret_cast<const float*>(a);ap[b]=batch0+b<batches ? unsigned(__half_as_ushort(__float2half_rn(raw[i])))|(unsigned(__half_as_ushort(__float2half_rn(raw[i+4])))<<16):0;}
                else ap[b]=batch0+b<batches ? unsigned(a[i])|(unsigned(a[i+4])<<16) : 0;}
            }
            #pragma unroll
            for(int r=0;r<R;r++)for(int c=0;c<4;c++)wa[r][c]=(tile+r)*32+lane<rows ? words[((tile+r)*groups+g)*128+c*32+lane]:0;
            if constexpr(Mode%12==9){
                #pragma unroll
                for(int c=0;c<4;c++){
                    float av[B][8];
                    #pragma unroll
...
__global__ void prepare_paired(const float* raw,uint16_t* a,unsigned pairs){
 unsigned p=blockIdx.x*blockDim.x+threadIdx.x;if(p>=pairs)return;
 unsigned i=(p/16)*32+(p%16)/4*8+p%4;
 reinterpret_cast<half2*>(a)[p]=__floats2half2_rn(raw[i],raw[i+4]);
}
