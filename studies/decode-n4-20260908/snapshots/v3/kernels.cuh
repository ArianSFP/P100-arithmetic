    ...
    for(unsigned stripe=warp;stripe<S/P;stripe+=NW){
        ...
        for(unsigned g=stripe*P+blockIdx.z;g<groups;g+=S){
            ...
            for(int b=0;b<B;b++){
                ...
                else{unsigned i=((batch0+b)*groups+g)*32+(lane&15)/4*8+(lane&3);if constexpr(Mode>=12){const float*raw=reinterpret_cast<const float*>(a);ap[b]=batch0+b<batches ? unsigned(__half_as_ushort(__float2half_rn(raw[i])))|(unsigned(__half_as_ushort(__float2half_rn(raw[i+4])))<<16):0;}
