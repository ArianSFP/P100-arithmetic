...
uint32_t rng(uint32_t &s)''')
    ...
    if(scalestress&&(scalestress!=1||rawwitness||
        (only!="wide_packed_split_u32"&&only!="wide_f32_split_u32"&&only!="compact_packed_u32"&&only!="compact_scalar_u32"&&only!="mid_packed_u32"&&only!="tile256_cap3"&&only!="compact_vstore"&&only!="compact_down_spec"&&only!="compact_gu_spec")))
    ...
    s=once(s,'    std::vector<float>reference,sequential;', '''    if(scalestress){
        ...
        cfg.erase(std::remove_if(cfg.begin(),cfg.end(),[](const Config&c){
            return c.kind!=-4&&c.kind!=-5&&c.column!=9032&&c.column!=10032&&c.column!=11032&&c.column!=12032&&c.column!=13032&&c.column!=14032&&c.column!=18032&&c.column!=19032&&c.column!=20032;
        }),cfg.end());
