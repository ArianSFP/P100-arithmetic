...
uint32_t rng(uint32_t &s)''')
    ...
    s=once(s,'    if(c.kind==1&&c.column>=7000){', '''    if(c.kind==1&&c.column>=11000){
        ...
        else if(c.column==15032)tile256_q4<32,4><<<grid,256>>>(leadraw,leadw,out,t,n,k,e);
    ...
    cfg.push_back({64,64,256,2,1,32,1,"tile256_cap4",15032});
    if(!only.empty())cfg.erase''')
    ...
    if(scalestress&&(scalestress!=1||rawwitness||
        (only!="wide_packed_split_u32"&&only!="wide_f32_split_u32"&&only!="compact_packed_u32"&&only!="compact_scalar_u32"&&only!="mid_packed_u32"&&only!="tile256_cap3"&&only!="tile256_cap4")))
    ...
    s=once(s,'    std::vector<float>reference,sequential;', '''    if(scalestress){
        ...
        cfg.erase(std::remove_if(cfg.begin(),cfg.end(),[](const Config&c){
            return c.kind!=-4&&c.kind!=-5&&c.column!=9032&&c.column!=10032&&c.column!=11032&&c.column!=12032&&c.column!=13032&&c.column!=14032&&c.column!=15032;
        }),cfg.end());
    ...
    for name,body in zip(sections[1::2],sections[2::2]):
        if 'HMUL2' in body:assert 'decode_check_kernel' in name or ((re.search(r'(?:wide|compact)_q4ILi\d+ELb1E',name) or 'tile256_q4' in name) and 'FFMA' in body),name
    hashes=old['hashes']|{str(p):digest(p) for p in [SOURCE,Path(__file__),ROOT/'staging.cuh',ROOT/'wide.cuh',ROOT/'compact.cuh',ROOT.parent/'w4a16-tile256-20260908/tile256.cuh',ROOT/'supervise.py',build/'worker.cu',build/'current_control.inc',build/'aw_control.inc',build/'aw_q4_control.inc',build/'worker']}
