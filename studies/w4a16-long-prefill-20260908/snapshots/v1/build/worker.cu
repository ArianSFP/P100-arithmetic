...
int main(int argc,char**argv) try {
    ...
    for(int i=1;i<argc;i+=2){if(i+1==argc)throw std::runtime_error("missing argument");std::string key=argv[i];
        ...
        if(key=="--gpu-approved")approved=v;else if(key=="--raw-witness")rawwitness=v;else if(key=="--prep-grid")prepgrid=v;else if(key=="--vector-prep")vectorprep=v;else if(key=="--tokens")t=v;else if(key=="--n")n=v;else if(key=="--k")k=v;
