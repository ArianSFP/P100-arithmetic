...
template<int U,bool PACKED,bool SPLIT=false>
__global__ __launch_bounds__(128)
void wide_q4(const float *a,const unsigned char *w,float *out,int t,int n,int k,int experts){
    __shared__ float sa[32][64],sb[32][128];
    const int tid=threadIdx.x,warp=tid/32,lane=tid%32;
    const int ar=(warp/2)*32+(lane/8)*8,bc=(warp%2)*64+(lane%8)*8;
    const int mt=(t+63)/64,ng=n/128,groups=k/32;
    for(int job=blockIdx.x;job<experts*mt*ng;job+=gridDim.x){
        ...
        float acc[8][8]={},acc_hi[SPLIT?8:1][SPLIT?8:1]={};fetch(0);stage();__syncthreads();
        for(int g=0;g<groups;++g){
            ...
            for(int chunk=0;chunk<32/U;++chunk){
                #pragma unroll
                for(int s=0;s<U;++s){
                    const int kk=chunk*U+s;float av[8],bv[8];
                    #pragma unroll
                    for(int j=0;j<8;j+=4){
        ...
        #pragma unroll
        for(int i=0;i<8;++i)if(tm*64+ar+i<t){
            #pragma unroll
            for(int j=0;j<8;++j){
                if constexpr(SPLIT)out[(size_t(e)*t+tm*64+ar+i)*n+col*128+bc+j]=acc[i][j]+acc_hi[i][j];
                else out[(size_t(e)*t+tm*64+ar+i)*n+col*128+bc+j]=acc[i][j];
            }
        }
        __syncthreads();
    }
