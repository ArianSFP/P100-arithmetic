...
uint32_t rng(uint32_t &s)''')
    ...
    s=once(s,'    std::vector<float>reference,sequential;', '''    if(scalestress){
        ...
        cfg.erase(std::remove_if(cfg.begin(),cfg.end(),[](const Config&c){
            return c.kind!=-4&&c.kind!=-5&&c.column!=9032&&c.column!=10032;
        }),cfg.end());
    ...
    for name,body in zip(sections[1::2],sections[2::2]):
        if 'HMUL2' in body:assert 'decode_check_kernel' in name or (re.search(r'wide_q4ILi\d+ELb1E',name) and 'FFMA' in body),name
    hashes=old['hashes']|{str(p):digest(p) for p in [SOURCE,Path(__file__),ROOT/'staging.cuh',ROOT/'wide.cuh',ROOT/'supervise.py',build/'worker.cu',build/'current_control.inc',build/'aw_control.inc',build/'aw_q4_control.inc',build/'worker']}
