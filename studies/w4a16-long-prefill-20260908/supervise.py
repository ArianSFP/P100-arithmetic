#!/usr/bin/env python3
"""GPU3-only guarded reservation. Controller never initializes CUDA."""
...
def main():
    if sys.argv[1:]!=['--gpu3-approved']:raise SystemExit('Explicit GPU3 authorization required')
    path=ROOT.parent/'w4a16-t64-r1-20260908/supervise.py'
    spec=importlib.util.spec_from_file_location('guard',path)
    guard=importlib.util.module_from_spec(spec);spec.loader.exec_module(guard)
    guard.ROOT=ROOT;guard.TOKEN='w4a16-long-prefill-20260908-gpu3-r7'
    guard.UUID='GPU-4868830a-c1cf-90bd-8018-2360c55293b8'
    def health():
        ...
        if len(c)!=7 or c[0]!=guard.UUID or 'P100' not in c[1] or int(c[2])>64 or int(c[6]):raise RuntimeError('GPU3 health: '+raw)
        ...
        for line in apps.splitlines():
            ...
            if fields[0]==guard.UUID:raise RuntimeError('GPU3 unexpected client: '+line)
        ...
        while True:
            try:return original_flock(fd,operation)
                ...
                if time.monotonic()>notice:
                    print('WAITING for shared benchmark lock; GPU3 not acquired',flush=True)
