...
uint32_t rng(uint32_t &s)''')
    ...
    for name,body in zip(sections[1::2],sections[2::2]):
        if 'HMUL2' in body:assert 'decode_check_kernel' in name or ((re.search(r'(?:wide|compact)_q4ILi\d+ELb1E',name) or re.search(r'tile256_(?:liveness_)?q4|tile256_direct_stage_q4|tile512_one_rail_q4',name)) and 'FFMA' in body),name
    hashes=old['hashes']|{str(p):digest(p) for p in [SOURCE,Path(__file__),ROOT/'staging.cuh',ROOT/'wide.cuh',ROOT/'compact.cuh',ROOT.parent/'w4a16-tile256-20260908/tile256.cuh',ROOT.parent/'w4a16-tile256-20260908/liveness.cuh',ROOT.parent/'w4a16-tile256-20260908/direct_stage.cuh',ROOT/'supervise.py',build/'worker.cu',build/'current_control.inc',build/'aw_control.inc',build/'aw_q4_control.inc',build/'worker']}
    rail_header=ROOT.parent/'w4a16-tile256-20260908/one_rail512.cuh'
    hashes[str(rail_header)]=digest(rail_header)
