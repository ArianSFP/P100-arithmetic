...
#define CANDIDATES(X) \
    X(64,64,256,2,0,8) X(64,64,256,2,0,16) \
    X(64,64,256,1,0,8) X(64,64,128,1,0,8) \
    X(64,64,128,2,0,8) X(64,64,128,1,1,8) \
    X(64,128,256,1,0,8) X(64,128,256,1,0,16) \
    X(64,128,256,1,1,8) X(64,128,128,1,1,8) \
    X(128,64,256,1,0,8) X(128,64,256,1,1,8) \
    X(32,128,128,1,0,8) X(32,128,128,1,1,8)
...
int main(int argc,char**argv) try {
    ...
    CANDIDATES(ADD)
    #undef ADD
    if(!only.empty())cfg.erase(std::remove_if(cfg.begin(),cfg.end(),[&](const Config&c){return c.kind==0&&c.name!=only;}),cfg.end());
    if(cfg.size()<4)throw std::runtime_error("no candidate");
    auto run=[&](const Config&c,bool prep){if(prep&&c.kind!=-3)prepare_a16<<<std::min<size_t>((ac+255)/256,4096),256>>>(da.p,dh.p,ac);
