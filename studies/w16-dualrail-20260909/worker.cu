...
int main(int argc,char**argv) try {
    ...
    Device<half>da(ac),dw16(wc);Device<unsigned char>dw8(packed.size());Device<float>out(oc);
    ...
    auto run=[&](int mode){
        if(mode==0){
            aw_q8_service_m64_n128_halfpipe_sync<true><<<original_grid,256>>>(dd8.p,dt.p,tiles.size(),n,k);
        } else {
            w8_make_pairs<<<1,256>>>(dt.p,tiles.size(),dtpairs.p,dtleft.p,dcounts.p);
            if(mode==8){
                w8_fallback_half<true,0><<<fallback_grid,256>>>(dd8.p,dtleft.p,tiles.size(),n,k,dcounts.p+1);
                if(n==512&&k==2048)w8_pair_half<true,0,512,2048><<<pair_grid,256>>>(dd8.p,dtpairs.p,(tiles.size()+1)/2,n,k,dcounts.p);
                else if(n==2048&&k==512)w8_pair_half<true,0,2048,512><<<pair_grid,256>>>(dd8.p,dtpairs.p,(tiles.size()+1)/2,n,k,dcounts.p);
                else w8_pair_half<true,0><<<pair_grid,256>>>(dd8.p,dtpairs.p,(tiles.size()+1)/2,n,k,dcounts.p);
            } else {
    ...
    printf("META M=%d N=%d K=%d experts=%d seed=3911 pair_grid=%d all_dispatch_launches_timed=1\n",m,n,k,ex,pair_grid);
