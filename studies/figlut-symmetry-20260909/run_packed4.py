...
for coord in coords:
    assert "FIGLU symmetry follow-up ACTIVE — GPU1 RESERVED" in coord.read_text(), coord
...
def health():
    ...
    if values[0] != "GPU-ecc6a1f9-42fe-2932-7ea0-1dd285491b7b" or int(values[-1]) != 0 or int(values[3]) > 80:
...
inputs = ["packed4_worker.cu", "packed4_worker", "packed4_worker.sass", "packed4_build.log", "packed4_sass_counts.json"]
hashes = {name: hashlib.sha256((root / name).read_bytes()).hexdigest() for name in inputs}
initial = health()
note("HELD: figlut-packed4-full16-r1; GPU1 device lock; corrected non-hoistable half2 control for activation-derived packed-four-token exact-G32 gate.")
meta = {"tag": tag, "device_index": 1, "device_uuid": "GPU-ecc6a1f9-42fe-2932-7ea0-1dd285491b7b",
        "initial_gpu": initial, "started": time.time(), "hashes": hashes,
        "runner_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
command = ["env", "-i", "PATH=/usr/local/cuda-12.8/bin:/usr/bin:/bin",
           ...
           "taskset", "--cpu-list", "0-11", str(root / "packed4_worker"), "--gpu-approved", "1", "256"]
...
try:
    ...
    note(f"FINAL RELEASE: figlut-packed4-full16-r1; GPU1 worker lock released, exit={meta.get('exit_code')}, reservation retained, no reset.")
    lock.close()
print("PASS", tag, "GPU1")
