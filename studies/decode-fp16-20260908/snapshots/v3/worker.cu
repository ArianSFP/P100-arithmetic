...
#undef ALLOC
 ...
 float oracle(const Cfg&c,unsigned row,unsigned b){float acc[32]={};for(int stripe=0;stripe<c.s;stripe++){float h0=0,h1=0;for(unsigned z=stripe;z<g;z+=c.s){auto&v=q[size_t(row)*g+z];float dot=0,x0=0,x1=0;for(int jj=0;jj<32;jj++){int j=c.mode>=3 ? (jj/8)*8+(jj%8)/2+(jj%2)*4 : jj;float x=half_float(a[b*k+z*32+j]),weight=native_code(v,j);if(c.mode<0||c.mode%3==0)dot=std::fma(weight,x,dot);else if(jj&1)x1=hf(double(weight)*x+x1);else x0=hf(double(weight)*x+x0);}float d=half_float(v.d);if(c.mode%3==2){h0=hf(double(x0)*d+h0);h1=hf(double(x1)*d+h1);}else{if(c.mode%3==1)dot=x0+x1;acc[stripe]=std::fma(dot,d,acc[stripe]);}}if(c.mode%3==2)acc[stripe]=h0+h1;}return tree_sum(acc,c.s);}
};
int main(int argc,char**argv){
 ...
 globalsets<1>();if(n>1)globalsets<4>();if(n>4){globalsets<8>();cfgs.push_back({"r3b8",-1,1,8,4,32,(void*)r3_batch8,false});}sets<1>();if(n>1)sets<4>();if(n>4){add<1,8,4,8>();add<1,8,8,16>();add<1,8,8,32>();add<2,8,8,8>();add<2,8,8,16>();}
