...
__device__ __forceinline__ half2 pair_bits(unsigned u) { __half2_raw raw;raw.x=uint16_t(u);raw.y=uint16_t(u>>16);return half2(raw); }
...
__global__ void decode(const Q4Block *native,const uint32_t *__restrict__ words,const uint16_t *__restrict__ scales,const uint16_t *__restrict__ a,float *__restrict__ out,unsigned rows,unsigned groups,unsigned batches) {
    ...
    for(unsigned stripe=warp;stripe<S;stripe+=NW){
        ...
        for(unsigned g=stripe;g<groups;g+=S){
            ...
            #pragma unroll
            for(int b=0;b<B;b++)ap[b]=batch0+b<batches ? reinterpret_cast<const unsigned*>(a)[((batch0+b)*groups+g)*16+(lane&15)] : 0;
            #pragma unroll
            ...
            for(int c=0;c<4;c++){
                ...
                for(int j=0;j<4;j++){
                    ...
                    #pragma unroll
                    for(int r=0;r<R;r++){
                        half2 q=nibble_pair(wa[r][c]>>(j*8));
                        #pragma unroll
                        for(int b=0;b<B;b++){
                            if constexpr(Mode==0){
            ...
            for(int r=0;r<R;r++){
                ...
                #pragma unroll
                for(int b=0;b<B;b++){
                    if constexpr(Mode==2)total[r][b]=__hfma2(hd[r][b],__halves2half2(d,d),total[r][b]);
                    else {float v=Mode==0 ? dot[r][b] : __low2float(hd[r][b])+__high2float(hd[r][b]);acc[r][b]=__fmaf_rn(v,__half2float(d),acc[r][b]);}
                }
            }
        }
        #pragma unroll
        for(int r=0;r<R;r++)for(int b=0;b<B;b++)sm[((b*S+stripe)*R+r)*32+lane]=Mode==2 ? __low2float(total[r][b])+__high2float(total[r][b]):acc[r][b];
    }
    __syncthreads();
    #pragma unroll
